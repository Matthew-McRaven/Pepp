...
std::unique_ptr<char *[]> ArrayFromWordList(char *wordlist, size_t slen, size_t *len, bool onlyLineEnds = false) {
  ...
  for (int j = 0; wordlist[j]; j++) {
    ...
    if (!wordSeparator[curr] && wordSeparator[prev]) words++;
    prev = curr;
  }
...
bool cmpWords(const char *a, const char *b) noexcept { return strcmp(a, b) < 0; }

} // namespace

WordList::WordList(bool onlyLineEnds_) noexcept : words(nullptr), list(nullptr), len(0), onlyLineEnds(onlyLineEnds_) {
...
WordList::operator bool() const noexcept { return len != 0; }

bool WordList::operator!=(const WordList &other) const noexcept {
  if (len != other.len) return true;
  for (size_t i = 0; i < len; i++) {
    if (strcmp(words[i], other.words[i]) != 0) return true;
  }
  return false;
}

int WordList::Length() const noexcept { return static_cast<int>(len); }
 ...
 */
bool WordList::InList(const std::string &s) const noexcept { return InList(s.c_str()); }
    ...
    while (words[j][0] == first) {
      ...
      while (*a && *a == *b) {
        ...
        if (*a == marker) {
          ...
          if (suffixLengthA >= suffixLengthB) break;
...
const char *WordList::WordAt(int n) const noexcept { return words[n]; }
