...
template <typename T> class LEXILLA_EXPORT OptionSet {
  ...
  struct Option {
    ...
    Option() : opType(SC_TYPE_BOOLEAN), pb(nullptr) {}
    Option(plcob pb_, std::string_view description_ = "")
        : opType(SC_TYPE_BOOLEAN), pb(pb_), description(description_) {}
    Option(plcoi pi_, std::string_view description_) : opType(SC_TYPE_INTEGER), pi(pi_), description(description_) {}
    Option(plcos ps_, std::string_view description_) : opType(SC_TYPE_STRING), ps(ps_), description(description_) {}
...
public:
  ...
  template <typename E> void DefineProperty(const char *name, E T::*pe, std::string_view description = "") {
  ...
  const char *PropertyNames() const noexcept { return names.c_str(); }
  ...
  const char *DescribeWordListSets() const noexcept { return wordLists.c_str(); }
};
} // namespace Lexilla
