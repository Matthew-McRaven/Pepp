...
class DiagramPropertyModel : public QAbstractListModel
{
    ...
    //               diagramChanged)
    //Q_PROPERTY(bool selected READ selected WRITE setSelected NOTIFY diagramChanged)

    //QList<DiagramProperty *> _properties;
    QHash<quint32, DiagramProperty *> _properties;
...
public:
    ...
    //Q_INVOKABLE bool selected(quint32 id) const;
    //Q_INVOKABLE void setSelected(quint32 id);
...
class DiagramSelectionModel : public QItemSelectionModel
{
    Q_OBJECT
    QML_ELEMENT
...
public:
    enum Behavior { DisabledBehavior, SelectSingle, SelectCells };
    Q_ENUM(Behavior)

    explicit DiagramSelectionModel(QObject *parent = nullptr) {};
...
private:
    Behavior _behavior = Behavior::DisabledBehavior;
};
