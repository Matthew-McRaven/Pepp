...
Decimal converters have two subtypes:
* signed
* unsigned
...
* Must only accept numbers in `[-2**(N-1),2**(N-1)-1]`
...
* At any time, typing a '-' shall toggle the sign.
  * Due to 2's complement magic, 0 and -2**(N-1) shall map to themselves, all other values should toggle signs.