...
ScrollView {
    ...
    property alias editorFont: quickScintillaEditor.font
    ...
    Item {
        ...
        ScintillaEditBase {
            ...
            width: root.availableWidth
                   - root.ScrollBar.vertical.width //+ 2*quickScintillaEditor.charHeight
            height: root.availableHeight
                    - root.ScrollBar.horizontal.height //+ 2*quickScintillaEditor.charWidth
    ...
    */
    ...
    Menu {
        ...
        Instantiator {
            ...
            delegate: MenuItem {
                ...
                onTriggered: menuCommandDelegate !== undefined ? menuCommandDelegate(
                                                                     model.menuId) : quickScintillaEditor.cmdContextMenu(
                                                                     model.menuId)
            }

            onObjectAdded: (index, object) => editContextMenu.insertItem(index,
                                                                         object)
    ...
    Connections {
        ...
        function onContentXChanged() {
            ...
            var deltaInColumns = parseInt(
                        delta / quickScintillaEditor.charWidth, 10)
            if (delta >= quickScintillaEditor.charWidth) {
                ...
                if (!root.actionFromKeyboard) {
                    quickScintillaEditor.enableUpdate(false)
                    quickScintillaEditor.x = quickScintillaEditor.x + deltaInColumns
                            * quickScintillaEditor.charWidth // deltaInColumns is < 0
                    if (quickScintillaEditor.x < 0) {
                        quickScintillaEditor.x = 0
                    }
                    quickScintillaEditor.scrollColumn(
                                deltaInColumns) // deltaInColumns is < 0
                    quickScintillaEditor.enableUpdate(true)
                }
            } else {
        ...
        function onContentYChanged() {
            ...
            var deltaInLines = parseInt(
                        delta / quickScintillaEditor.charHeight, 10)
            if (delta >= quickScintillaEditor.charHeight) {
                // disable repaint: https://stackoverflow.com/questions/46095768/how-to-disable-update-on-a-qquickitem
                quickScintillaEditor.enableUpdate(false)
                quickScintillaEditor.y = quickScintillaEditor.y + deltaInLines
                        * quickScintillaEditor.charHeight // TODO --> bewirkt geometry changed !!!
                quickScintillaEditor.scrollRow(deltaInLines)
                quickScintillaEditor.enableUpdate(true)
            } else if (-delta >= quickScintillaEditor.charHeight) {
                quickScintillaEditor.enableUpdate(false)
                quickScintillaEditor.y = quickScintillaEditor.y + deltaInLines
                        * quickScintillaEditor.charHeight
    ...
    Connections {
        ...
        function onAddToContextMenu(menuId, txt, enabled) {
        ...
        function onClearContextMenu() {
            contextMenuModel.clear()
        }

        function onDoubleClick() {//console.log("double click !")
        }

        function onMarginClicked() {//console.log("MARGING CLICK !")
        }

        function onTextAreaClicked() {//console.log("TextArea CLICK !")
        }
