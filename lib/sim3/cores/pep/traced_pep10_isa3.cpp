...
targets::pep10::isa::CPU::CPU(sim::api2::device::Descriptor device, sim::api2::device::IDGenerator gen)
    : _device(device), _regs({.id = gen(), .baseName = "regs", .fullName = _device.fullName + "/regs"},
                             AddressSpan<u8>(0, u8(::isa::Pep10::RegisterCount * 2 - 1))),
      _csrs({.id = gen(), .baseName = "csrs", .fullName = _device.fullName + "/csrs"},
            AddressSpan<u8>(0, u8(::isa::Pep10::CSRCount - 1))) {}

sim::api2::memory::Target<u8> *targets::pep10::isa::CPU::regs() { return &_regs; }

sim::api2::memory::Target<u8> *targets::pep10::isa::CPU::csrs() { return &_csrs; }
...
std::optional<u16> targets::pep10::isa::CPU::currentOperand() {
...
u16 targets::pep10::isa::CPU::startingPC() const { return _startingPC; }

void targets::pep10::isa::CPU::updateStartingPC() { _startingPC = readReg(::isa::Pep10::Register::PC); }

u16 targets::pep10::isa::CPU::depth() const { return _depth; }
...
void targets::pep10::isa::CPU::setTarget(sim::api2::memory::Target<u16> *target, void *port) { _memory = target; }
...
void targets::pep10::isa::CPU::setCallsViaRet(const QSet<u16> &calls) { _callsViaRet = calls; }
...
u16 targets::pep10::isa::CPU::readReg(::isa::Pep10::Register reg) {
  u16 ret = 0;
  targets::isa::readRegister<::isa::Pep10, u8>(&_regs, reg, ret, rw_d);
  return ret;
}

void targets::pep10::isa::CPU::writeReg(::isa::Pep10::Register reg, u16 val) {
  targets::isa::writeRegister<::isa::Pep10, u8>(&_regs, reg, val, rw_d);
}
...
void targets::pep10::isa::CPU::writeCSR(::isa::Pep10::CSR csr, bool val) {
  targets::isa::writeCSR<::isa::Pep10, u8>(&_csrs, csr, val, rw_d);
}

u8 targets::pep10::isa::CPU::readPackedCSR() {
  u8 ret = 0;
  targets::isa::readPackedCSR<::isa::Pep10>(&_csrs, ret, rw_d);
  return ret;
}

void targets::pep10::isa::CPU::writePackedCSR(u8 val) {
  targets::isa::writePackedCSR<::isa::Pep10>(&_csrs, val, rw_d);
}

sim::api2::tick::Result targets::pep10::isa::CPU::unaryDispatch(u8 is, u16 pc) {
  using ISA = ::isa::Pep10;
  ...
  case mn::SRET:
    ...
    if (swap) tmp = bits::byteswap(tmp);
    _memory->write(static_cast<u16>(::isa::Pep10::MemoryVectors::SystemStackPtr),
  ...
  case mn::SCALL:
    ...
    ctx[9] = is;
    _memory->read(pc, {reinterpret_cast<u8 *>(&tmp), 2}, rw_i);
    bits::memcpy(ctxSpan.subspan(10, 2), tmpSpan);

    // Read system stack address.
    _memory->read(static_cast<u16>(::isa::Pep10::MemoryVectors::SystemStackPtr),
                  {reinterpret_cast<u8 *>(&tmp), 2}, rw_d);
    if (swap) tmp = bits::byteswap(tmp);
    ...
    _memory->read(static_cast<u16>(::isa::Pep10::MemoryVectors::TrapHandler), {reinterpret_cast<u8 *>(&tmp), 2},
                  rw_d);
    if (swap) tmp = bits::byteswap(tmp);
    pc = tmp;
    incrDepth();
...
sim::api2::tick::Result targets::pep10::isa::CPU::nonunaryDispatch(u8 is, u16 os, u16 pc) {
  using ISA = ::isa::Pep10;
  using mn = ISA::Mnemonic;
  using Register = ISA::Register;
...
void targets::pep10::isa::CPU::decodeStoreOperand(u8 is, u16 os, u16 &decoded, bool traced) {
  using Register = ::isa::Pep10::Register;
...
void targets::pep10::isa::CPU::decodeLoadOperand(u8 is, u16 os, u16 &decoded, bool traced) {
  using Register = ::isa::Pep10::Register;
