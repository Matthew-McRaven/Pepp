...
public:
  ...
  sim::memory::Input<u16> *input(QString name);
  QStringList outputs() const;
  sim::memory::Output<u16> *output(QString name);
...
private:
  ...
  struct ReloadHelper {
    QSharedPointer<sim::api2::memory::Target<u16>> target;
    u16 base;
    std::vector<u8> data;
  };
  void appendReloadEntries(QSharedPointer<sim::api2::memory::Target<u16>> mem, const obj::MemoryRegion &reg,
                           u16 baseOffset = 0);
  ...
  QVector<QSharedPointer<sim::memory::Dense<u16>>> _rawMemory = {};
  QVector<QSharedPointer<sim::memory::ReadOnly<u16>>> _readonly = {};

  QMap<QString, QSharedPointer<sim::memory::Input<u16>>> _mmi = {};
  QMap<QString, QSharedPointer<sim::memory::Output<u16>>> _mmo = {};
  QMap<QString, QSharedPointer<sim::memory::IDEController>> _ide = {};
  QMap<sim::api2::device::ID, sim::api2::device::Descriptor> _devices = {};
};

bool loadRegion(sim::api2::memory::Target<u16> &mem, const obj::MemoryRegion &reg, u16 baseOffset = 0);
// Do not buffer any MMIO values.
bool loadElfSegments(sim::api2::memory::Target<u16> &mem, const ELFIO::elfio &elf);
