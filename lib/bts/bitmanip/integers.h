...
template <typename T, bool is_le, int size = sizeof(T)> class Integer {
public:
  constexpr Integer() = default;

  constexpr Integer(T v) {
    if (std::is_constant_evaluated() || size == 3) {
      for (int i = 0; i < size; i++) buf[is_le ? i : (size - i - 1)] = v >> (i * 8);
    } else {
      if (!is_native) v = bswap(v);
      memcpy(buf, &v, size);
    }
  }
  ...
  operator T() const {
    if constexpr (size == 3) {
      if (is_le) return buf[2] << 16 | buf[1] << 8 | buf[0];
