#pragma once
#include <QtCore>
#include <QtQmlIntegration>
#include <vector>
#include "core/ds/linenumbers.hpp"
// Extend existing pepp::Line2Address with the difference between listing and source lines.
struct Lines2Addresses {
  Lines2Addresses() {};
  Lines2Addresses(std::vector<std::pair<int, u32>> source, std::vector<std::pair<int, u32>> list);
  std::optional<u32> source2Address(int sourceLine);
  std::optional<u32> list2Address(int listLine);
  std::optional<int> address2Source(u32 address);
  std::optional<int> address2List(u32 address);
  std::optional<int> source2List(int source);
  std::optional<int> list2Source(int list);

private:
  friend class ScopedLines2Addresses;
  pepp::Line2Address _source, _listing;
};

class ScopedLines2Addresses : public QObject {
  Q_OBJECT
  QML_ELEMENT

public:
  using scope = int;
  ScopedLines2Addresses(QObject *parent = nullptr);
  ~ScopedLines2Addresses() = default;
  void addScope(QString name);
  void addScope(QString name, const Lines2Addresses &map);
  std::optional<QString> scope2name(scope) const;
  std::optional<scope> name2scope(QString) const;
  std::optional<u32> source2Address(int sourceLine, scope) const;
  std::optional<u32> list2Address(int listLine, scope) const;
  std::optional<u32> source2Address(int sourceLine, QString) const;
  std::optional<u32> list2Address(int listLine, QString) const;
  std::optional<int> source2List(int source, scope) const;
  std::optional<int> list2Source(int list, scope) const;

  std::optional<std::tuple<scope, int>> address2Source(u32 address) const;
  std::optional<std::tuple<scope, int>> address2List(u32 address) const;

public slots:
  void onReset();

signals:
  void wasReset();

private:
  // Use scope as an index into this vector-of-maps. Line numbers are repeated between scopes, so we cannot easily
  // combine the maps.
  std::vector<std::map<int, u32>> _source2Addr{}, _list2Addr{};
  // Addresses are unique, so we can fuse all scopes into a single map.
  std::map<u32, std::tuple<scope, int>> _addr2Source{}, _addr2List{};
  std::map<scope, QString> _scopeNames;
  scope add_or_get_scope(QString name);
};
