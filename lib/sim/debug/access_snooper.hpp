...
template <typename Addr> Decoded decodeRead(bits::span<const u8> payload, sim::api2::packet::Flags flags) {
...
template <typename Addr> Decoded decodeWriteThrough(bits::span<const u8> payload, sim::api2::packet::Flags flags) {
...
Decoded decodeAddressedWrite(bits::span<const u8> payload, sim::api2::packet::Flags flags) {
...
Decoded decode(bits::span<const u8> payload, sim::api2::packet::Flags flags) {
  switch ((u16)flags) {
  case 0b0000'0000: // Empty Packet
    return {.success = true, .address = 0, .length = 0};
  case 0b0010'0000: // Read Packet,           1B Address
    return decodeRead<u8>(payload, flags);
  case 0b0010'0100: // Write Packet           1B address, 1B data
    return decodeAddressedWrite<u8, u8>(payload, flags);
  case 0b0010'1000: // Write Packet           1B address, 2B data
    return decodeAddressedWrite<u8, u8[2]>(payload, flags);
  case 0b0010'1100: // Write Packet           1B address, 4B data
    return decodeAddressedWrite<u8, u8[4]>(payload, flags);
  case 0b0011'0000: // Write Packet           1B address, 8B data
    return decodeAddressedWrite<u8, u8[8]>(payload, flags);
  case 0b0011'0100: // Write Packet           1B address, 16B data
    return decodeAddressedWrite<u8, u8[16]>(payload, flags);
  case 0b0011'1000: // Write Packet           1B address, 32B data
    return decodeAddressedWrite<u8, u8[32]>(payload, flags);
  case 0b0011'1100: // Write Packet           1B address, 64B data
    return decodeAddressedWrite<u8, u8[64]>(payload, flags);
  case 0b0100'0000: // Read Packet,           2B Address
    return decodeRead<u16>(payload, flags);
  case 0b0100'0100: // Write Packet           2B address, 1B data
    return decodeAddressedWrite<u16, u8>(payload, flags);
  case 0b0100'1000: // Write Packet           2B address, 2B data
    return decodeAddressedWrite<u16, u8[2]>(payload, flags);
  case 0b0100'1100: // Write Packet           2B address, 4B data
    return decodeAddressedWrite<u16, u8[4]>(payload, flags);
  case 0b0101'0000: // Write Packet           2B address, 8B data
    return decodeAddressedWrite<u16, u8[8]>(payload, flags);
  case 0b0101'0100: // Write Packet           2B address, 16B data
    return decodeAddressedWrite<u16, u8[16]>(payload, flags);
  case 0b0101'1000: // Write Packet           2B address, 32B data
    return decodeAddressedWrite<u16, u8[32]>(payload, flags);
  case 0b0101'1100: // Write Packet           2B address, 64B data
    return decodeAddressedWrite<u16, u8[64]>(payload, flags);
  case 0b0110'0000: // Read Packet,           4B Address
    return decodeRead<u32>(payload, flags);
  case 0b0110'0100: // Write Packet           4B address, 1B data
    return decodeAddressedWrite<u32, u8>(payload, flags);
  case 0b0110'1000: // Write Packet           4B address, 2B data
    return decodeAddressedWrite<u32, u8[2]>(payload, flags);
  case 0b0110'1100: // Write Packet           4B address, 4B data
    return decodeAddressedWrite<u32, u8[4]>(payload, flags);
  case 0b0111'0000: // Write Packet           4B address, 8B data
    return decodeAddressedWrite<u32, u8[8]>(payload, flags);
  case 0b0111'0100: // Write Packet           4B address, 16B data
    return decodeAddressedWrite<u32, u8[16]>(payload, flags);
  case 0b0111'1000: // Write Packet           4B address, 32B data
    return decodeAddressedWrite<u32, u8[32]>(payload, flags);
  case 0b0111'1100: // Write Packet           4B address, 64B data
    return decodeAddressedWrite<u32, u8[64]>(payload, flags);
  case 0b1000'0000: // Read Packet,           8B Address
    return decodeRead<u64>(payload, flags);
  case 0b1000'0100: // Write Packet           8B address, 1B data
    return decodeAddressedWrite<u64, u8>(payload, flags);
  case 0b1000'1000: // Write Packet           8B address, 2B data
    return decodeAddressedWrite<u64, u8[2]>(payload, flags);
  case 0b1000'1100: // Write Packet           8B address, 4B data
    return decodeAddressedWrite<u64, u8[4]>(payload, flags);
  case 0b1011'0000: // Write Packet           8B address, 8B data
    return decodeAddressedWrite<u64, u8[8]>(payload, flags);
  case 0b1011'0100: // Write Packet           8B address, 16B data
    return decodeAddressedWrite<u64, u8[16]>(payload, flags);
  case 0b1011'1000: // Write Packet           8B address, 32B data
    return decodeAddressedWrite<u64, u8[32]>(payload, flags);
  case 0b1011'1100: // Write Packet           8B address, 64B data
    return decodeAddressedWrite<u64, u8[64]>(payload, flags);
  case 0b1010'0000: // Writethrough packet,   1B address
    return decodeWriteThrough<u8>(payload, flags);
  case 0b1010'0100: // Writethrough packet,   2B address
    return decodeWriteThrough<u16>(payload, flags);
  case 0b1010'1000: // Writethrough packet,   4B address
    return decodeWriteThrough<u32>(payload, flags);
  case 0b1010'1100: // Writethrough packet,   8B address
    return decodeWriteThrough<u64>(payload, flags);
  }
...
public:
  ...
  // Analyzer interface
  bool analyze(bits::span<const u8> payload, api2::packet::Flags flags) override;
  bool unanalyze(bits::span<const u8> payload, api2::packet::Flags flags) override;
...
bool sim::debug::AccessSnooper<Address>::analyze(bits::span<const u8> payload, api2::packet::Flags flags) {
...
bool sim::debug::AccessSnooper<Address>::unanalyze(bits::span<const u8> payload, api2::packet::Flags flags) {
  auto decoded = detail::decode(payload, flags);
...
template <typename Address> void AccessSnooper<Address>::printData(detail::Decoded decoded, bool dir) {
  ...
  if (decoded.success && decoded.length > 0) {
    ...
    u8 data[16];
    std::span<u8> dataSpan = data;
    u64 it = 0;
    while (it < decoded.length) {
      auto count = std::min<u64>(sizeof(data), decoded.length - it);
