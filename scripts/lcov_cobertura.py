#!/usr/bin/env python
# Taken from: https://github.com/eriwen/lcov-to-cobertura-xml
    ...
    def parse(self):
        ...
        for line in self.lcov_data.split('\n'):
            ...
            if input_type == 'SF':
                ...
                # FN:5,(anonymous_1)
                parts = line_parts[-1].strip().split(',')
                function_line, function_name = parts[0], parts[-1]
                file_methods[function_name] = [function_line, '0']
            elif input_type == 'FNDA':
                # FNDA:0,(anonymous_1)
                parts = line_parts[-1].strip().split(',')
                (function_hits, function_name) = parts[0], parts[-1]
