...
int Accessor::GetPropertyInt(std::string_view key, int defaultValue) const { return pprops->GetInt(key, defaultValue); }
...
int Accessor::IndentAmount(Sci_Position line, int *flags, PFNIsCommentLeader pfnIsCommentLeader) {
  ...
  while ((ch == ' ' || ch == '\t') && (pos < end)) {
    if (inPrevPrefix) {
      ...
      if (chPrev == ' ' || chPrev == '\t') {
        if (chPrev != ch) spaceFlags |= wsInconsistent;
      } else {
        inPrevPrefix = false;
      }
    }
    if (ch == ' ') {
      ...
      spaceFlags |= wsTab;
      if (spaceFlags & wsSpace) spaceFlags |= wsSpaceTab;
      indent = (indent / 8 + 1) * 8;
    }
    ch = (*this)[++pos];
  }
