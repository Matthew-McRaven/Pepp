...
template <typename T> class SparseState {
  struct State {
    ...
    T value;
    constexpr State(Sci_Position position_, T value_) noexcept : position(position_), value(std::move(value_)) {}
    inline bool operator<(const State &other) const noexcept { return position < other.position; }
...
public:
  explicit SparseState(Sci_Position positionFirst_ = -1) { positionFirst = positionFirst_; }
  ...
  T ValueAt(Sci_Position position) {
    ...
    if (position < states[0].position) return T();
  ...
  bool Merge(const SparseState<T> &other, Sci_Position ignoreAfter) {
    ...
    if (different) {
      ...
      if (!states.empty() && !other.states.empty() && states.back().value == startOther->value) ++startOther;
