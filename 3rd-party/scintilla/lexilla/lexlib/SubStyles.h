...
class LEXILLA_EXPORT WordClassifier {
  int baseStyle;
  int firstStyle;
  int lenStyles;
...
public:
  ...
  int Base() const noexcept { return baseStyle; }

  int Start() const noexcept { return firstStyle; }

  int Last() const noexcept { return firstStyle + lenStyles - 1; }

  int Length() const noexcept { return lenStyles; }
  ...
  int ValueFor(std::string_view s) const {
    ...
    if (it != wordToStyle.end()) return it->second;
    else return -1;
  }

  bool IncludesStyle(int style) const noexcept { return (style >= firstStyle) && (style < (firstStyle + lenStyles)); }
...
class LEXILLA_EXPORT SubStyles {
  int classifications;
  ...
  int BlockFromBaseStyle(int baseStyle) const noexcept {
    for (int b = 0; b < classifications; b++) {
      if (baseStyle == baseStyles[b]) return b;
    }
    return -1;
  }
  ...
  int BlockFromStyle(int style) const noexcept {
    ...
    for (const WordClassifier &wc : classifiers) {
      if (wc.IncludesStyle(style)) return b;
      b++;
    }
    return -1;
  }
...
public:
  SubStyles(const char *baseStyles_, int styleFirst_ = SubStylesFirst, int stylesAvailable_ = SubStylesAvailable,
            int secondaryDistance_ = 0)
  ...
  int Allocate(int styleBase, int numberStyles) noexcept {
    ...
    if (block >= 0) {
      if ((allocated + numberStyles) > stylesAvailable) return -1;
  ...
  int BaseStyle(int subStyle) const noexcept {
    ...
    if (block >= 0) return classifiers[block].Base();
    else return subStyle;
  }

  int DistanceToSecondaryStyles() const noexcept { return secondaryDistance; }

  int FirstAllocated() const noexcept {
    int start = 257;
    for (const WordClassifier &wc : classifiers) {
      if ((wc.Length() > 0) && (start > wc.Start())) start = wc.Start();
    }
  ...
  int LastAllocated() const noexcept {
    int last = -1;
    for (const WordClassifier &wc : classifiers) {
      if ((wc.Length() > 0) && (last < wc.Last())) last = wc.Last();
    }
    return last;
  }
  ...
  void SetIdentifiers(int style, const char *identifiers) {
    ...
    if (block >= 0) classifiers[block].SetIdentifiers(style, identifiers);
  }
