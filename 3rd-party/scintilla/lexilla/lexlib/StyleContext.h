...
class LEXILLA_EXPORT StyleContext {
  LexAccessor &styler;
  ...
  void GetNextChar() {
    ...
    if (currentLine < lineDocEnd) atLineEnd = currentPosSigned >= (lineStartNext - 1);
    else // Last line
...
public:
  ...
  bool More() const noexcept { return currentPos < endPos; }
  void Forward() {
  ...
  void ChangeState(int state_) noexcept { state = state_; }
  ...
  Sci_Position LengthCurrent() const noexcept { return currentPos - styler.GetStartSegment(); }
  ...
  int GetRelativeCharacter(Sci_Position n) {
    if (n == 0) return ch;
    if (multiByteAccess) {
      if ((currentPosLastRelative != currentPos) || ((n > 0) && ((offsetRelative < 0) || (n < offsetRelative))) ||
  ...
  bool Match(const char *s) {
    ...
    if (ch != su) return false;
    s++;
    if (!*s) return true;
    const unsigned char sNext = *s;
    if (chNext != sNext) return false;
    s++;
    for (int n = 2; *s; n++) {
      if (*s != styler.SafeGetCharAt(currentPos + n, 0)) return false;
      s++;
    }
    return true;
  }
