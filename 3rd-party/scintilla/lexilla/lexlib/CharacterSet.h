...
public:
  ...
  CharacterSetArray(setBase base = setNone, const char *initialSet = "", bool valueAfter_ = false) noexcept {
    ...
    if (base & setLower) AddString("abcdefghijklmnopqrstuvwxyz");
    if (base & setUpper) AddString("ABCDEFGHIJKLMNOPQRSTUVWXYZ");
    if (base & setDigits) AddString("0123456789");
  }
...
LEXILLA_EXPORT int CompareCaseInsensitive(const char *a, const char *b) noexcept;
LEXILLA_EXPORT bool EqualCaseInsensitive(std::string_view a, std::string_view b) noexcept;
LEXILLA_EXPORT int CompareNCaseInsensitive(const char *a, const char *b, size_t len) noexcept;
