...
enum class WorkItems { none = 0, style = 1, updateUI = 2 };

class SCINTILLA_EXPORT WorkNeeded {
public:
  ...
  void Need(WorkItems items_, Sci::Position pos) noexcept {
    if (Scintilla::FlagSet(items_, WorkItems::style) && (upTo < pos)) upTo = pos;
 ...
 */
class SCINTILLA_EXPORT SelectionText {
  std::string s;
...
public:
  ...
  SelectionText() noexcept
      : rectangular(false), lineCopy(false), codePage(0), characterSet(Scintilla::CharacterSet::Ansi) {}
  void Clear() noexcept {
    s.clear();
    rectangular = false;
    lineCopy = false;
  ...
  void Copy(const std::string &s_, int codePage_, Scintilla::CharacterSet characterSet_, bool rectangular_,
            bool lineCopy_) {
    s = s_;
    codePage = codePage_;
  ...
  const char *Data() const noexcept { return s.c_str(); }
  size_t Length() const noexcept { return s.length(); }
  size_t LengthWithTerminator() const noexcept { return s.length() + 1; }
...
struct SCINTILLA_EXPORT WrapPending {
  ...
  bool NeedsWrap() const noexcept { return start < end; }
...
struct SCINTILLA_EXPORT CaretPolicySlop {
  Scintilla::CaretPolicy
      policy; // Combination from CaretPolicy::Slop, CaretPolicy::Strict, CaretPolicy::Jumps, CaretPolicy::Even
...
struct SCINTILLA_EXPORT CaretPolicies {
  CaretPolicySlop x;
  CaretPolicySlop y;
};

struct SCINTILLA_EXPORT VisiblePolicySlop {
...
/**
 */
class SCINTILLA_EXPORT Editor : public EditModel, public DocWatcher {
protected: // ScintillaBase subclass needs access to much of Editor
  ...
  SelectionPosition SPositionFromLocation(Point pt, bool canReturnInvalid = false, bool charPosition = false,
  ...
  SelectionPosition MovePositionOutsideChar(SelectionPosition pos, Sci::Position moveDir,
                                            bool checkLineEnd = true) const;
  void MovedCaret(SelectionPosition newPos, SelectionPosition previousPos, bool ensureVisible, CaretPolicies policies);
  void MovePositionTo(SelectionPosition newPos, Selection::SelTypes selt = Selection::SelTypes::none,
                      bool ensureVisible = true);
  void MovePositionTo(Sci::Position newPos, Selection::SelTypes selt = Selection::SelTypes::none,
                      bool ensureVisible = true);
  ...
  virtual std::unique_ptr<Surface> CreateMeasurementSurface(Scintilla::Internal::PainterID pid = nullptr) const;
  virtual std::unique_ptr<Surface> CreateDrawingSurface(SurfaceID sid,
                                                        std::optional<Scintilla::Technology> technologyOpt = {}) const;
  ...
  virtual Scintilla::sptr_t DefWndProc(Scintilla::Message iMessage, Scintilla::uptr_t wParam,
  ...
  static void *PtrFromSPtr(Scintilla::sptr_t lParam) noexcept { return reinterpret_cast<void *>(lParam); }
  ...
  static char *CharPtrFromSPtr(Scintilla::sptr_t lParam) noexcept { return static_cast<char *>(PtrFromSPtr(lParam)); }
  ...
  static void *PtrFromUPtr(Scintilla::uptr_t wParam) noexcept { return reinterpret_cast<void *>(wParam); }
  ...
  static constexpr Sci::Position PositionFromUPtr(Scintilla::uptr_t wParam) noexcept { return SPtrFromUPtr(wParam); }
  static constexpr Sci::Line LineFromUPtr(Scintilla::uptr_t wParam) noexcept { return SPtrFromUPtr(wParam); }
  ...
  template <typename T> bool SetAppearance(T &variable, T value) {
 ...
 */
class SCINTILLA_EXPORT AutoSurface {
private:
  std::unique_ptr<Surface> surf;

public:
  AutoSurface(const Editor *ed, Scintilla::Internal::PainterID pid = nullptr) : surf(ed->CreateMeasurementSurface()) {
    surf->Init(false, pid);
  }
  ...
  ~AutoSurface() {}
  Surface *operator->() const noexcept { return surf.get(); }
  operator Surface *() const noexcept { return surf.get(); }
};
