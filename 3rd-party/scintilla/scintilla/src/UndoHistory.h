...
struct SCINTILLA_EXPORT SizeMax {
  size_t size = 1;
  size_t maxValue = UINT8_MAX;
};

class SCINTILLA_EXPORT ScaledVector {
  SizeMax element;
...
class SCINTILLA_EXPORT UndoActionType {
public:
  ActionType at : 4;
  bool mayCoalesce : 1;
...
struct SCINTILLA_EXPORT UndoActions {
...
class SCINTILLA_EXPORT ScrapStack {
  std::string stack;
  size_t current = 0;
...
class SCINTILLA_EXPORT UndoHistory {
  UndoActions actions;
  int currentAction = 0;
...
public:
  ...
  const char *AppendAction(ActionType at, Sci::Position position, const char *data, Sci::Position lengthData,
                           bool &startSequence, bool mayCoalesce = true);
