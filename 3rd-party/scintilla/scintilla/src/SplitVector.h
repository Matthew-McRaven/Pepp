      ...
      try {
        if (gapLength > 0) { // If gap to move
          ...
          if (position < part1Length) {
            ...
            std::move_backward(body.data() + position, body.data() + part1Length,
                               body.data() + gapLength + part1Length);
          } else { // position > part1Length
            // Moving the gap towards end so moving elements towards start
            std::move(body.data() + part1Length + gapLength, body.data() + gapLength + position,
...
public:
  ...
  size_t GetGrowSize() const noexcept { return growSize; }

  void SetGrowSize(size_t growSize_) noexcept { growSize = growSize_; }
  ...
  template <typename ParamType> void SetValueAt(ptrdiff_t position, ParamType &&v) noexcept {
    if (position < part1Length) {
  ...
  ptrdiff_t Length() const noexcept { return lengthBody; }
  ...
  void GetRange(T *buffer, ptrdiff_t position, ptrdiff_t retrieveLength) const {
    ...
    if (position < part1Length) {
      ...
      if (range1Length > part1AfterPosition) range1Length = part1AfterPosition;
    }
  ...
  ptrdiff_t GapPosition() const noexcept { return part1Length; }
};
