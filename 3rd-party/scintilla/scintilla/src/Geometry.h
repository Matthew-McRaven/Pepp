...
public:
  ...
  constexpr bool operator==(Point other) const noexcept { return (x == other.x) && (y == other.y); }
 ...
 */
class SCINTILLA_EXPORT Interval {
public:
  XYPOSITION left;
  XYPOSITION right;
  ...
  constexpr bool Empty() const noexcept { return Width() <= 0; }
  constexpr bool Intersects(Interval other) const noexcept { return (right > other.left) && (left < other.right); }
  constexpr Interval Offset(XYPOSITION offset) const noexcept { return {left + offset, right + offset}; }
};
 ...
 */
class SCINTILLA_EXPORT PRectangle {
public:
  XYPOSITION left;
  XYPOSITION top;
  XYPOSITION right;
  ...
  static constexpr PRectangle FromInts(int left_, int top_, int right_, int bottom_) noexcept {
    return PRectangle(static_cast<XYPOSITION>(left_), static_cast<XYPOSITION>(top_), static_cast<XYPOSITION>(right_),
                      static_cast<XYPOSITION>(bottom_));
  }
  ...
  constexpr Point Centre() const noexcept { return Point((left + right) / 2, (top + bottom) / 2); }
...
SCINTILLA_EXPORT PRectangle Clamp(PRectangle rc, Edge edge, XYPOSITION position) noexcept;
SCINTILLA_EXPORT PRectangle Side(PRectangle rc, Edge edge, XYPOSITION size) noexcept;

SCINTILLA_EXPORT Interval Intersection(Interval a, Interval b) noexcept;
SCINTILLA_EXPORT PRectangle Intersection(PRectangle rc, Interval horizontalBounds) noexcept;
SCINTILLA_EXPORT Interval HorizontalBounds(PRectangle rc) noexcept;

SCINTILLA_EXPORT XYPOSITION PixelAlign(XYPOSITION xy, int pixelDivisions) noexcept;
SCINTILLA_EXPORT XYPOSITION PixelAlignFloor(XYPOSITION xy, int pixelDivisions) noexcept;
SCINTILLA_EXPORT XYPOSITION PixelAlignCeil(XYPOSITION xy, int pixelDivisions) noexcept;

SCINTILLA_EXPORT Point PixelAlign(const Point &pt, int pixelDivisions) noexcept;

SCINTILLA_EXPORT PRectangle PixelAlign(const PRectangle &rc, int pixelDivisions) noexcept;
SCINTILLA_EXPORT PRectangle PixelAlignOutside(const PRectangle &rc, int pixelDivisions) noexcept;
...
class SCINTILLA_EXPORT ColourRGBA {
  static constexpr float ComponentAsFloat(unsigned char component) { return component / componentMaximum; }
...
public:
  ...
  static constexpr ColourRGBA FromRGB(int co_) noexcept { return ColourRGBA(co_ | (maximumByte << 24)); }
  ...
  constexpr ColourRGBA WithoutAlpha() const noexcept { return ColourRGBA(co & rgbMask); }

  constexpr ColourRGBA Opaque() const noexcept { return ColourRGBA(co | (maximumByte << 24)); }

  constexpr int AsInteger() const noexcept { return co; }

  constexpr int OpaqueRGB() const noexcept { return co & rgbMask; }

  // Red, green and blue values as bytes 0..255
  constexpr unsigned char GetRed() const noexcept { return co & maximumByte; }
  constexpr unsigned char GetGreen() const noexcept { return (co >> 8) & maximumByte; }
  constexpr unsigned char GetBlue() const noexcept { return (co >> 16) & maximumByte; }
  ...
  constexpr float GetRedComponent() const noexcept { return ComponentAsFloat(GetRed()); }
  constexpr float GetGreenComponent() const noexcept { return ComponentAsFloat(GetGreen()); }
  constexpr float GetBlueComponent() const noexcept { return ComponentAsFloat(GetBlue()); }
  constexpr float GetAlphaComponent() const noexcept { return ComponentAsFloat(GetAlpha()); }

  constexpr bool operator==(const ColourRGBA &other) const noexcept { return co == other.co; }

  constexpr bool IsOpaque() const noexcept { return GetAlpha() == maximumByte; }
 ...
 */
class SCINTILLA_EXPORT Stroke {
public:
  ColourRGBA colour;
  XYPOSITION width;
  constexpr Stroke(ColourRGBA colour_, XYPOSITION width_ = 1.0) noexcept : colour(colour_), width(width_) {}
  constexpr float WidthF() const noexcept { return static_cast<float>(width); }
};

/**
 * Holds an RGBA colour to fill a shape.
 */
class SCINTILLA_EXPORT Fill {
public:
  ColourRGBA colour;
  constexpr Fill(ColourRGBA colour_) noexcept : colour(colour_) {}
};

/**
 * Holds a pair of RGBA colours and stroke width to fill and stroke a shape.
 */
class SCINTILLA_EXPORT FillStroke {
public:
  Fill fill;
  Stroke stroke;
 ...
 */
class SCINTILLA_EXPORT ColourStop {
public:
  XYPOSITION position;
  ColourRGBA colour;
  constexpr ColourStop(XYPOSITION position_, ColourRGBA colour_) noexcept : position(position_), colour(colour_) {}
};
} // namespace Scintilla::Internal
