...
template <typename DISTANCE> struct FillResult {
  bool changed;
  DISTANCE position;
  DISTANCE fillLength;
};

template <typename DISTANCE, typename STYLE> class RunStyles {
private:
...
DISTANCE Scintilla::Internal::RunStyles<DISTANCE, STYLE>::RunFromPosition(DISTANCE position) const noexcept {
...
template <typename DISTANCE, typename STYLE>
DISTANCE Scintilla::Internal::RunStyles<DISTANCE, STYLE>::SplitRun(DISTANCE position) {
...
template <typename DISTANCE, typename STYLE>
void Scintilla::Internal::RunStyles<DISTANCE, STYLE>::RemoveRun(DISTANCE run) {
  starts.RemovePartition(run);
  styles.DeleteRange(run, 1);
}

template <typename DISTANCE, typename STYLE>
void Scintilla::Internal::RunStyles<DISTANCE, STYLE>::RemoveRunIfEmpty(DISTANCE run) {
...
template <typename DISTANCE, typename STYLE>
void Scintilla::Internal::RunStyles<DISTANCE, STYLE>::RemoveRunIfSameAsPrevious(DISTANCE run) {
...
template <typename DISTANCE, typename STYLE> Scintilla::Internal::RunStyles<DISTANCE, STYLE>::RunStyles() {
...
template <typename DISTANCE, typename STYLE>
DISTANCE Scintilla::Internal::RunStyles<DISTANCE, STYLE>::Length() const noexcept {
  return starts.PositionFromPartition(starts.Partitions());
}

template <typename DISTANCE, typename STYLE>
STYLE Scintilla::Internal::RunStyles<DISTANCE, STYLE>::ValueAt(DISTANCE position) const noexcept {
  return styles.ValueAt(starts.PartitionFromPosition(position));
}

template <typename DISTANCE, typename STYLE>
DISTANCE Scintilla::Internal::RunStyles<DISTANCE, STYLE>::FindNextChange(DISTANCE position,
                                                                         DISTANCE end) const noexcept {
  ...
  if (run < starts.Partitions()) {
    ...
    if (runChange > position) return runChange;
...
template <typename DISTANCE, typename STYLE>
DISTANCE Scintilla::Internal::RunStyles<DISTANCE, STYLE>::StartRun(DISTANCE position) const noexcept {
  return starts.PositionFromPartition(starts.PartitionFromPosition(position));
}

template <typename DISTANCE, typename STYLE>
DISTANCE Scintilla::Internal::RunStyles<DISTANCE, STYLE>::EndRun(DISTANCE position) const noexcept {
  return starts.PositionFromPartition(starts.PartitionFromPosition(position) + 1);
}

template <typename DISTANCE, typename STYLE>
Scintilla::Internal::FillResult<DISTANCE>
Scintilla::Internal::RunStyles<DISTANCE, STYLE>::FillRange(DISTANCE position, STYLE value, DISTANCE fillLength) {
...
template <typename DISTANCE, typename STYLE>
void Scintilla::Internal::RunStyles<DISTANCE, STYLE>::SetValueAt(DISTANCE position, STYLE value) {
  FillRange(position, value, 1);
}

template <typename DISTANCE, typename STYLE>
void Scintilla::Internal::RunStyles<DISTANCE, STYLE>::InsertSpace(DISTANCE position, DISTANCE insertLength) {
  DISTANCE runStart = RunFromPosition(position);
  if (starts.PositionFromPartition(runStart) == position) {
    STYLE runStyle = ValueAt(position);
...
template <typename DISTANCE, typename STYLE> void Scintilla::Internal::RunStyles<DISTANCE, STYLE>::DeleteAll() {
  starts = Partitioning<DISTANCE>(8);
  styles = SplitVector<STYLE>();
  styles.InsertValue(0, 2, 0);
}
...
template <typename DISTANCE, typename STYLE>
void Scintilla::Internal::RunStyles<DISTANCE, STYLE>::DeleteRange(DISTANCE position, DISTANCE deleteLength) {
...
template <typename DISTANCE, typename STYLE>
DISTANCE Scintilla::Internal::RunStyles<DISTANCE, STYLE>::Runs() const noexcept {
  return starts.Partitions();
}

template <typename DISTANCE, typename STYLE>
bool Scintilla::Internal::RunStyles<DISTANCE, STYLE>::AllSame() const noexcept {
  for (DISTANCE run = 1; run < starts.Partitions(); run++) {
    const DISTANCE runBefore = run - 1;
    if (styles.ValueAt(run) != styles.ValueAt(runBefore)) return false;
  }
  return true;
}

template <typename DISTANCE, typename STYLE>
bool Scintilla::Internal::RunStyles<DISTANCE, STYLE>::AllSameAs(STYLE value) const noexcept {
  return AllSame() && (styles.ValueAt(0) == value);
}

template <typename DISTANCE, typename STYLE>
DISTANCE Scintilla::Internal::RunStyles<DISTANCE, STYLE>::Find(STYLE value, DISTANCE start) const noexcept {
  if (start < Length()) {
    DISTANCE run = start ? RunFromPosition(start) : 0;
    if (styles.ValueAt(run) == value) return start;
    run++;
    while (run < starts.Partitions()) {
      if (styles.ValueAt(run) == value) return starts.PositionFromPartition(run);
      run++;
    }
  }
  return -1;
...
template <typename DISTANCE, typename STYLE> void Scintilla::Internal::RunStyles<DISTANCE, STYLE>::Check() const {
  if (Length() < 0) {
