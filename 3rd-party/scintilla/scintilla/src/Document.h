 ...
 */
class SCINTILLA_EXPORT Range {
public:
  Sci::Position start;
  Sci::Position end;

  explicit Range(Sci::Position pos = 0) noexcept : start(pos), end(pos) {}
  Range(Sci::Position start_, Sci::Position end_) noexcept : start(start_), end(end_) {}

  bool operator==(const Range &other) const noexcept { return (start == other.start) && (end == other.end); }

  bool Valid() const noexcept { return (start != Sci::invalidPosition) && (end != Sci::invalidPosition); }

  [[nodiscard]] bool Empty() const noexcept { return start == end; }

  [[nodiscard]] Sci::Position Length() const noexcept { return (start <= end) ? (end - start) : (start - end); }

  Sci::Position First() const noexcept { return (start <= end) ? start : end; }

  Sci::Position Last() const noexcept { return (start > end) ? start : end; }
  ...
  bool Contains(Range other) const noexcept { return Contains(other.start) && Contains(other.end); }
 ...
 */
class SCINTILLA_EXPORT RegexSearchBase {
public:
  ...
  virtual Sci::Position FindText(Document *doc, Sci::Position minPos, Sci::Position maxPos, const char *s,
                                 bool caseSensitive, bool word, bool wordStart, Scintilla::FindOption flags,
...
struct SCINTILLA_EXPORT StyledText {
  size_t length;
  const char *text;
  bool multipleStyles;
  size_t style;
  ...
  size_t StyleAt(size_t i) const noexcept { return multipleStyles ? styles[i] : style; }
};

class SCINTILLA_EXPORT HighlightDelimiter {
public:
  HighlightDelimiter() noexcept : isEnabled(false) { Clear(); }
  ...
  bool IsHeadOfFoldBlock(Sci::Line line) const noexcept { return beginFoldBlock == line && line < endFoldBlock; }
  ...
  Sci::Line firstChangeableLineBefore; // First line that triggers repaint before starting line that determined current
                                       // fold block
  Sci::Line firstChangeableLineAfter;  // First line that triggers repaint after starting line that determined current
                                       // fold block
  bool isEnabled;
};

struct SCINTILLA_EXPORT LexerReleaser {
...
class SCINTILLA_EXPORT LexInterface {
protected:
  Document *pdoc;
...
struct SCINTILLA_EXPORT RegexError : public std::runtime_error {
...
class SCINTILLA_EXPORT ActionDuration {
  double duration;
 ...
 */
struct SCINTILLA_EXPORT CharacterExtracted {
...
/**
 */
class SCINTILLA_EXPORT Document : PerLine,
...
public:
  ...
  Sci::Position MovePositionOutsideChar(Sci::Position pos, Sci::Position moveDir,
                                        bool checkLineEnd = true) const noexcept;
  ...
  bool SetUndoCollection(bool collectUndo) noexcept { return cb.SetUndoCollection(collectUndo); }
  ...
  Sci::Line LineFromPositionIndex(Sci::Position pos,
  ...
  Sci::Position FindText(Sci::Position minPos, Sci::Position maxPos, const char *search, Scintilla::FindOption flags,
                         Sci::Position *length);
  ...
  Sci::Position BraceMatch(Sci::Position position, Sci::Position maxReStyle, Sci::Position startPos,
...
class SCINTILLA_EXPORT UndoGroup {
  Document *pdoc;
  bool groupNeeded;

public:
  UndoGroup(Document *pdoc_, bool groupNeeded_ = true) noexcept : pdoc(pdoc_), groupNeeded(groupNeeded_) {
    if (groupNeeded) {
  ...
  bool Needed() const noexcept { return groupNeeded; }
};
 ...
 */
class SCINTILLA_EXPORT DocModification {
public:
  ...
  DocModification(Scintilla::ModificationFlags modificationType_, Sci::Position position_ = 0,
                  Sci::Position length_ = 0, Sci::Line linesAdded_ = 0, const char *text_ = nullptr,
                  Sci::Line line_ = 0) noexcept
      : modificationType(modificationType_), position(position_), length(length_), linesAdded(linesAdded_), text(text_),
        line(line_), foldLevelNow(Scintilla::FoldLevel::None), foldLevelPrev(Scintilla::FoldLevel::None),
        annotationLinesAdded(0), token(0) {}
  ...
  DocModification(Scintilla::ModificationFlags modificationType_, const Action &act, Sci::Line linesAdded_ = 0) noexcept
      : modificationType(modificationType_), position(act.position), length(act.lenData), linesAdded(linesAdded_),
        text(act.data), line(0), foldLevelNow(Scintilla::FoldLevel::None), foldLevelPrev(Scintilla::FoldLevel::None),
        annotationLinesAdded(0), token(0) {}
};
 ...
 */
class SCINTILLA_EXPORT DocWatcher {
public:
  virtual ~DocWatcher() {}
