...
private:
  ...
  void ClearValue(Sci::Position partition) noexcept { values.SetValueAt(partition, T()); }
...
public:
  ...
  Sci::Position Length() const noexcept { return starts.Length(); }
  Sci::Position Elements() const noexcept { return starts.Partitions(); }
  ...
  template <typename ParamType> void SetValueAt(Sci::Position position, ParamType &&value) {
    assert(position <= Length());
    const Sci::Position partition = ElementFromPosition(position);
    const Sci::Position startPartition = starts.PositionFromPartition(partition);
    if (value == T()) {
