...
class SCINTILLA_EXPORT CharacterIndexer {
public:
  virtual char CharAt(Sci::Position index) const = 0;
  virtual Sci::Position MovePositionOutsideChar(Sci::Position pos,
                                                [[maybe_unused]] Sci::Position moveDir) const noexcept = 0;
};

class SCINTILLA_EXPORT RESearch {
...
private:
  ...
  bool iswordc(unsigned char x) const noexcept { return charClass->IsWord(x); }
};
} // namespace Scintilla::Internal
