...
struct SCINTILLA_EXPORT Chunk {
  size_t start;
  size_t end;
  constexpr Chunk(size_t start_ = 0, size_t end_ = 0) noexcept : start(start_), end(end_) { assert(start <= end); }
  size_t Length() const noexcept;
};

/**
 */
class SCINTILLA_EXPORT CallTip {
  ...
  int DrawChunk(Surface *surface, int x, std::string_view sv, int ytext, PRectangle rcClient, bool asHighlight,
                bool draw);
...
public:
  ...
  PRectangle CallTipStart(Sci::Position pos, Point pt, int textHeight, const char *defn, int codePage_,
