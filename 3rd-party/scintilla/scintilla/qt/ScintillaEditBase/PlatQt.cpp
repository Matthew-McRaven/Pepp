...
const char *CharacterSetID(CharacterSet characterSet) {
  switch (characterSet) {
    ...
    //	return "";
  case CharacterSet::Default: return "ISO 8859-1";
  case CharacterSet::Baltic: return "ISO 8859-13";
  case CharacterSet::ChineseBig5: return "Big5";
  case CharacterSet::EastEurope: return "ISO 8859-2";
  case CharacterSet::GB2312: return "GB18030-0";
  case CharacterSet::Greek: return "ISO 8859-7";
  case CharacterSet::Hangul: return "CP949";
  case CharacterSet::Mac:
    return "Apple Roman";
    // case SC_CHARSET_OEM:
    //	return "ASCII";
  case CharacterSet::Russian: return "KOI8-R";
  case CharacterSet::Cyrillic: return "Windows-1251";
  ...
  case CharacterSet::Turkish:
    ...
    //         return "CP1361";
  case CharacterSet::Hebrew: return "ISO 8859-8";
  case CharacterSet::Arabic: return "ISO 8859-6";
  case CharacterSet::Vietnamese: return "Windows-1258";
  case CharacterSet::Thai: return "TIS-620";
  case CharacterSet::Iso8859_15: return "ISO 8859-15";
  default: return "ISO 8859-1";
  }
}

QString UnicodeFromText(QStringDecoder *decoder, std::string_view text) { return decoder->decode(text); }
...
const FontAndCharacterSet *AsFontAndCharacterSet(const Font *f) { return dynamic_cast<const FontAndCharacterSet *>(f); }

QFont *FontPointer(const Font *f) { return AsFontAndCharacterSet(f)->pfont.get(); }

} // namespace

std::shared_ptr<Font> Font::Allocate(const FontParameters &fp) { return std::make_shared<FontAndCharacterSet>(fp); }
...
void SurfaceImpl::SetMode(SurfaceMode mode_) { mode = mode_; }

void SurfaceImpl::Release() noexcept { Clear(); }
...
void SurfaceImpl::BrushColour(ColourRGBA back) { GetPainter()->setBrush(QBrush(QColorFromColourRGBA(back))); }
...
void SurfaceImpl::SetCodec(const Font *font) {
  ...
  if (pfacs && pfacs->pfont) {
    ...
    if (!(mode.codePage == SC_CP_UTF8)) csid = CharacterSetID(pfacs->characterSet);
    if (csid != codecName) {
      codecName = csid;
      decoder = std::make_unique<QStringDecoder>(csid);
    }
  }
}
...
void SurfaceImpl::FillRectangleAligned(PRectangle rc, Fill fill) { FillRectangle(PixelAlign(rc, 1), fill); }
...
void SurfaceImpl::GradientRectangle(PRectangle rc, const std::vector<ColourStop> &stops, GradientOptions options) {
  ...
  switch (options) {
  case GradientOptions::leftToRight: linearGradient = QLinearGradient(rc.left, rc.top, rc.right, rc.top); break;
  case GradientOptions::topToBottom:
  default: linearGradient = QLinearGradient(rc.left, rc.top, rc.left, rc.bottom); break;
  }
...
static std::vector<unsigned char> ImageByteSwapped(int width, int height, const unsigned char *pixelsImage) {
  ...
  for (size_t i = 0; i < bytes; i += 4) std::swap(imageBytes[i], imageBytes[i + 2]);
  return imageBytes;
}
...
void SurfaceImpl::DrawTextNoClip(PRectangle rc, const Font *font, XYPOSITION ybase, std::string_view text,
...
void SurfaceImpl::DrawTextClipped(PRectangle rc, const Font *font, XYPOSITION ybase, std::string_view text,
                                  ColourRGBA fore, ColourRGBA back) {
  SetClip(rc);
...
void SurfaceImpl::DrawTextTransparent(PRectangle rc, const Font *font, XYPOSITION ybase, std::string_view text,
                                      ColourRGBA fore) {
  SetFont(font);
  PenColour(fore);
...
void SurfaceImpl::PopClip() { GetPainter()->restore(); }

void SurfaceImpl::MeasureWidths(const Font *font, std::string_view text, XYPOSITION *positions) {
  if (!font) return;
  SetCodec(font);
  QString su = UnicodeFromText(decoder.get(), text);
...
void SurfaceImpl::DrawTextNoClipUTF8(PRectangle rc, const Font *font, XYPOSITION ybase, std::string_view text,
                                     ColourRGBA fore, ColourRGBA back) {
  SetFont(font);
  PenColour(fore);
...
void SurfaceImpl::DrawTextClippedUTF8(PRectangle rc, const Font *font, XYPOSITION ybase, std::string_view text,
                                      ColourRGBA fore, ColourRGBA back) {
  SetClip(rc);
  DrawTextNoClip(rc, font, ybase, text, fore, back);
  PopClip();
...
void SurfaceImpl::DrawTextTransparentUTF8(PRectangle rc, const Font *font, XYPOSITION ybase, std::string_view text,
                                          ColourRGBA fore) {
  SetFont(font);
  PenColour(fore);
...
void SurfaceImpl::MeasureWidthsUTF8(const Font *font, std::string_view text, XYPOSITION *positions) {
  if (!font) return;
  QString su = QString::fromUtf8(text.data(), static_cast<int>(text.length()));
  QTextLayout tlay(su, *FontPointer(font), GetPaintDevice());
  tlay.beginLayout();
...
XYPOSITION SurfaceImpl::InternalLeading(const Font * /* font */) { return 0; }
...
QPaintDevice *SurfaceImpl::GetPaintDevice() { return device; }
...
#ifdef PLAT_QT_QML
QQuickPaintedItem *window(WindowID wid) noexcept { return static_cast<QQuickPaintedItem *>(wid); }
#else
QWidget *window(WindowID wid) noexcept { return static_cast<QWidget *>(wid); }
#endif
...
void Window::SetPositionRelative(PRectangle rc, const Window *relativeTo) {
  ...
  if (ox < rectDesk.x()) ox = rectDesk.x();
  if (sizex > screenWidth) ox = rectDesk.x(); /* the best we can do */
  else if (ox + sizex > rectDesk.right()) ox = rectDesk.right() - sizex;
  if (oy + sizey > rectDesk.bottom()) oy = rectDesk.bottom() - sizey;
  if (oy < rectDesk.top()) oy = rectDesk.top();
...
void Window::Show(bool show) {
  if (wid) window(wid)->setVisible(show);
}

void Window::InvalidateAll() {
  if (wid) window(wid)->update();
}

void Window::InvalidateRectangle(PRectangle rc) {
  if (wid) window(wid)->update(QRectFromPRect(rc));
}
...
public:
  ...
  void Create(Window &parent, int ctrlID, Point location, int lineHeight, bool unicodeMode_,
...
void ListBoxImpl::Create(Window &parent, int /*ctrlID*/, Point location, int /*lineHeight*/, bool unicodeMode_,
                         Technology) {
  ...
  foreach (QPixmap im, images) {
    if (maxIconWidth < im.width()) maxIconWidth = im.width();
    if (maxIconHeight < im.height()) maxIconHeight = im.height();
  }
...
void ListBoxImpl::SetVisibleRows(int rows) { visibleRows = rows; }

int ListBoxImpl::GetVisibleRows() const { return visibleRows; }
...
void ListBoxImpl::RegisterQPixmapImage(int type, const QPixmap &pm) {
  ...
  if (list) {
    ...
    if (pm.width() > iconSize.width() || pm.height() > iconSize.height())
      list->setIconSize(QSize(qMax(pm.width(), iconSize.width()), qMax(pm.height(), iconSize.height())));
  }
}
...
void ListBoxImpl::ClearRegisteredImages() {
  images.clear();
  ListWidget *list = GetWidget();
  if (list) list->setIconSize(QSize(0, 0));
}
...
ListWidget *ListBoxImpl::GetWidget() const noexcept { return static_cast<ListWidget *>(wid); }

ListBox::ListBox() noexcept = default;
ListBox::~ListBox() noexcept = default;

std::unique_ptr<ListBox> ListBox::Allocate() { return std::make_unique<ListBoxImpl>(); }
ListWidget::ListWidget(QWidget *parent) : QListWidget(parent), delegate(nullptr) {}

void ListWidget::setDelegate(IListBoxDelegate *lbDelegate) { delegate = lbDelegate; }
...
unsigned int Platform::DoubleClickTime() { return QApplication::doubleClickInterval(); }

void Scintilla::Internal::Platform::DebugDisplay(const char *s) noexcept { qWarning("Scintilla: %s", s); }

void Scintilla::Internal::Platform::DebugPrintf(const char *format, ...) noexcept {
  char buffer[2000];
  va_list pArguments{};
...
bool Scintilla::Internal::Platform::ShowAssertionPopUps(bool /*assertionPopUps*/) noexcept { return false; }

} // namespace Scintilla::Internal

void Scintilla::Internal::Platform::Assert(const char *c, const char *file, int line) noexcept {
  char buffer[2000];
