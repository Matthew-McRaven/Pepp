...
#include <cassert>
#include <cfloat>
// #include <cmath> // Does not work in in freestanding mode.
#include <cassert>
#include <cstdio>
#include <errno.h>
#include "./native_libc.h"

float test_fadd(float a, float b) { return a + b; }
float test_fsub(float a, float b) { return a - b; }
float test_fmul(float a, float b) { return a * b; }
float test_fdiv(float a, float b) { return a / b; }
float test_fmax(float a, float b) { return (a < b) ? b : a; }
float test_fmin(float a, float b) { return (a < b) ? a : b; }
double test_ftod(float val) { return (double)val; }
float test_dtof(double val) { return (float)val; }

float test_fneg(float val) { return -val; }
double test_dneg(double val) { return -val; }

float test_fmadd(float a, float b, float c) { return a * b + c; }
float test_fmsub(float a, float b, float c) { return a * b - c; }
float test_fnmadd(float a, float b, float c) { return -(a * b) + c; }
float test_fnmsub(float a, float b, float c) { return -(a * b) - c; }
...
// float test_fsqrt(float val) { return std::sqrt(val); }
// double test_dsqrt(double val) { return std::sqrt(val); }
// float test_fpow(float val, float exp) { return std::pow(val, exp); }
// double test_dpow(double val, double exp) { return std::pow(val, exp); }

// float test_sinf(float val) { return std::sin(val); }
// float test_cosf(float val) { return std::cos(val); }
// float test_tanf(float val) { return std::tan(val); }

// static const double PI = std::atan(1.0) * 4.0;

// These are largely arbitrary, but so is the exactness
// of certain algorithms...
inline bool kinda32(float val, float expectation) { return val >= expectation - 1e-4 && val < expectation + 1e-4; }
inline bool kinda64(float val, double expectation) { return val >= expectation - 1e-7 && val < expectation + 1e-7; }
