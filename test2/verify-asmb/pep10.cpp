...
TEST_CASE("CS6E figure assembly, Standalone", "[scope:asm][kind:e2e][arch:pep10]") {
...
TEST_CASE("CS6E figure assembly, Unified", "[scope:asm][kind:e2e][arch:pep10]") {
  auto fs = builtins::QtFilesystemProvider::create();
  auto bookReg = pepp::BuiltinRegistry(std::move(fs));
  auto book = bookReg.find_book("Computer Systems, 6th Edition");
  ...
  for (auto &fig : book->figures()) {
    ...
    DYNAMIC_SECTION(chapter.toStdString() << "." << figName.toStdString()) {
      ...
      if (!result) {
        ...
        for (auto &line : lines) qCritical() << line;
        for (auto &error : pas::ops::generic::collectErrors(*osRoot)) qCritical() << "OS:   " << error.second.message;
        for (auto &error : pas::ops::generic::collectErrors(*userRoot)) qCritical() << "USER: " << error.second.message;
      }
      auto elf = pas::obj::pep10::createElf();
      pas::obj::pep10::combineSections(*osRoot);
      pas::obj::pep10::writeOS(*elf, *osRoot);
