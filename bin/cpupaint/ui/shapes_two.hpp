...
const QRect AddrBus = QRect(QPoint(40, 151), QPoint(40 + 20, BottomOfAlu + MemReadYOffsetFromALU + 15));
...
const Arrow loadCkSelect =
    Arrow(QVector<QPoint>() << QPoint(RegBank.right() + arrowHOffset, 24),
          QVector<QLine>() << QLine(ctrlInputX - 7, loadCkCheckbox.y() + selectYOffset, RegBank.right() + arrowHOffset,
...
const Arrow CSelect =
    Arrow(QVector<QPoint>() << QPoint(RegBank.right() + arrowHOffset, 47),
          QVector<QLine>() << QLine(ctrlInputX - 7, cLabel.y() + selectYOffset, RegBank.right() + arrowHOffset,
...
const Arrow BSelect = Arrow(
    QVector<QPoint>() << QPoint(RegBank.right() + arrowHOffset, 69),
    QVector<QLine>() << QLine(ctrlInputX - 7, bLabel.y() + selectYOffset, RegBank.right() + arrowHOffset,
...
const Arrow ASelect = Arrow(
    QVector<QPoint>() << QPoint(RegBank.right() + arrowHOffset, 91),
    QVector<QLine>() << QLine(ctrlInputX - 7, aLabel.y() + selectYOffset, RegBank.right() + arrowHOffset,
...
const Arrow MARMuxSelect = Arrow(
    QVector<QPoint>() << QPoint(MARMuxerDataLabel.x() + MARMuxerDataLabel.width() / 2 - 3, MARMuxerDataLabel.y() - 12),
    QVector<QLine>()
        // Horizontal line from middle of MARMux to the tristate label
        << QLine(MARMuxerDataLabel.x() + MARMuxerDataLabel.width() / 2,
                 MARMuxTristateLabel.y() + MARMuxTristateLabel.height() / 2, ctrlInputX - 7,
                 MARMuxTristateLabel.y() + MARMuxTristateLabel.height() / 2)
...
const QRect MARCkCheckbox =
    QRect(ctrlInputX, MARMuxerDataLabel.y() + MARMuxerDataLabel.height() / 2 - check2H / 2, check2W, check2H);
...
const QPolygon MARMuxToMARABus = QPolygon(
    QVector<QPoint>()
    ...
    << QPoint(MARALabel.right() + arrowHDepth - 5,
              MARALabel.y() + MARALabel.height() / 2 + 5) // Arrow Bottom Inner point
    << QPoint(MARALabel.right() + arrowHDepth - 5,
              MARALabel.y() + MARALabel.height() / 2 + 10)                              // Arrow Bottom Outer point
    ...
    << QPoint(MARALabel.right() + arrowHDepth - 5,
              MARALabel.y() + MARALabel.height() / 2 - 5)); // Arrow Top Inner point

const QPolygon MARMuxToMARBBus = QPolygon(
    QVector<QPoint>()
    ...
    << QPoint(MARBLabel.right() + arrowHDepth - 5,
              MARBLabel.y() + MARALabel.height() / 2 + 5) // Arrow Bottom Inner point
    << QPoint(MARBLabel.right() + arrowHDepth - 5,
              MARBLabel.y() + MARALabel.height() / 2 + 10)                              // Arrow Bottom Outer point
    ...
    << QPoint(MARBLabel.right() + arrowHDepth - 5,
              MARBLabel.y() + MARALabel.height() / 2 - 5)); // Arrow Top Inner point
...
const Arrow MDROck = Arrow(QVector<QPoint>() << QPoint(MDROLabel.x() + MDROLabel.width() / 2 - 3, MDROLabel.y() - 12),
                           QVector<QLine>()
                               ...
                               << QLine(MDROLabel.x() + MDROLabel.width() / 2,
                                        MDROCkCheckbox.y() + MDROCkCheckbox.height() / 2, ctrlInputX - 7,
                                        MDROCkCheckbox.y() + MDROCkCheckbox.height() / 2)
...
const Arrow MDROMuxSelect =
    Arrow(QVector<QPoint>() << QPoint(MDROMuxerDataLabel.right() + 5,
...
const QRect MDRELabel =
    QRect(combCircX, MDROMuxerDataLabel.bottom() + MDRORegOffsetFromMDREMux, dataLabelW, dataLabelH);
const QRect MDRECkCheckbox = QRect(ctrlInputX, MDRELabel.y() - 40, checkW + 10, checkH);
const Arrow MDREck = Arrow(QVector<QPoint>() << QPoint(MDRELabel.x() + MDRELabel.width() / 2 - 3, MDRELabel.y() - 12),
                           QVector<QLine>()
                               // Horizontal line from MDRECk to midpoint of MDREven
                               << QLine(ctrlInputX - 7, MDRECkCheckbox.y() + MDRECkCheckbox.height() / 2,
                                        MDRELabel.x() + MDRELabel.width() / 2,
                                        MDRECkCheckbox.y() + MDRECkCheckbox.height() / 2)
                               // Vertical line connecting arrowhead and horizontal line segment
                               << QLine(MDRELabel.x() + MDRELabel.width() / 2,
                                        MDRECkCheckbox.y() + MDRECkCheckbox.height() / 2,
...
const Arrow MDREMuxSelect =
    Arrow(QVector<QPoint>() << QPoint(MDREMuxerDataLabel.right() + 5,
                                      MDREMuxerDataLabel.y() + MDREMuxerDataLabel.height() / 2 - 3),
          QVector<QLine>()
...
const QRect EOMuxerDataLabel =
    QRect(MARMuxerDataLabel.x() + MARMuxerDataLabel.width() / 2 - dataLabelW / 2, // Center EOMux horizontally on MARMux
...
const Arrow EOMuxSelect =
    Arrow(QVector<QPoint>() << QPoint(EOMuxerDataLabel.right() + 4, EOMuxTristateLabel.y() + 6),
          QVector<QLine>() << QLine(EOMuxerDataLabel.right() + 5, EOMuxerDataLabel.y() + EOMuxerDataLabel.height() / 2,
...
const Arrow CMuxSelect = Arrow(
    QVector<QPoint>() << QPoint(cMuxerLabel.left() + 7, cMuxerLabel.top() - 12),
...
const QPolygon MARBus = QPolygon(
    QVector<QPoint>()
    // Top Foot
    << QPoint(MARBLabel.x() + MARBLabel.width() / 2 - 5, MARBLabel.bottom() + 1) // Foot Top Left point
    << QPoint(MARBLabel.x() + MARBLabel.width() / 2 - 5,
              (MARBLabel.bottom() + MARALabel.y()) / 2 - 10) // P ivot between foot right corner and arrow inner point.
    // arrow:
    ...
    // Bottom Foot
    << QPoint(MARALabel.x() + MARBLabel.width() / 2 - 5,
              (MARBLabel.bottom() + MARALabel.y()) / 2 + 10) // Pivot between bottom right corner and bottom inner point
    ...
    // Black Line
    << QPoint(MARBLabel.x() + MARBLabel.width() / 2 + 5,
              (MARBLabel.bottom() + MARALabel.y()) / 2)                                // Black line right point
...
const QRect aMuxerDataLabel =
    QRect(((controlOffsetX + OneByteShapes::ALUUpperLeftLine_LeftPoint) +
          ...
          ALUPoly.boundingRect().y() - AMuxYOffsetFromALUPoly, dataLabelW,
          dataLabelH); // Place AMuxYOffsetFromALUPoly pixels distance between AMux and the ALU
...
const Arrow AMuxSelect =
    Arrow(QVector<QPoint>()
              ...
              << QPoint(aMuxerDataLabel.x() + aMuxerDataLabel.width() + 3,
                        aMuxerDataLabel.y() + aMuxerDataLabel.height() / 2 - 2),
          ...
          QVector<QLine>() << QLine(ctrlInputX - 7, aMuxerDataLabel.y() + aMuxerDataLabel.height() / 2 + 1,
                                    ...
                                    aMuxerDataLabel.x() + aMuxerDataLabel.width() + 5,
                                    aMuxerDataLabel.y() + aMuxerDataLabel.height() / 2 + 1));

// EOMux bus definition is split from EOMux, because it depends on AMUX, which depends on the ALU
const QPolygon EOMuxOutputBus = QPolygon(
    QVector<QPoint>()
    // Foot
    << QPoint(EOMuxerDataLabel.x() + EOMuxerDataLabel.width() / 2 - 5,
              EOMuxerDataLabel.bottom() + 1) // Foot Top Left point
    << QPoint(EOMuxerDataLabel.x() + EOMuxerDataLabel.width() / 2 + 5,
              EOMuxerDataLabel.bottom() + 1) // Foot Top Right point
    << QPoint(EOMuxerDataLabel.x() + EOMuxerDataLabel.width() / 2 + 5,
              EOMuxerDataLabel.bottom() + 10) // Pivot between upper right vertical leg and upper horizontal leg
    << QPoint(aMuxerDataLabel.x() + 15,
              EOMuxerDataLabel.bottom() + 10) // Pivot between upper horizontal leg, and the right vertical leg
    // Arrow
    ...
    // Remainder of Foot
    << QPoint(aMuxerDataLabel.x() + 5,
              EOMuxerDataLabel.bottom() + 20) // Pivot between arrow left side and lower horizontal leg
    << QPoint(EOMuxerDataLabel.x() + EOMuxerDataLabel.width() / 2 - 5,
              EOMuxerDataLabel.bottom() + 20) // Pivot between lower horizontal leg and start
);

const QPolygon AMuxBus =
    QPolygon(QVector<QPoint>() << QPoint(aMuxerDataLabel.x() + aMuxerDataLabel.width() / 2 - 5,
                                         aMuxerDataLabel.y() + aMuxerDataLabel.height()) // Foot Upper Left point
                               << QPoint(aMuxerDataLabel.x() + aMuxerDataLabel.width() / 2 + 5,
                                         aMuxerDataLabel.y() + aMuxerDataLabel.height()) // Upper Right point
                               << QPoint(aMuxerDataLabel.x() + aMuxerDataLabel.width() / 2 + 5,
                                         ALUPoly.boundingRect().y() - (arrowHDepth - 5)) // Arrow Right Inner point
                               << QPoint(aMuxerDataLabel.x() + aMuxerDataLabel.width() / 2 + 10,
                                         ALUPoly.boundingRect().y() - (arrowHDepth - 5)) // Arrow Right Outer point
                               << QPoint(aMuxerDataLabel.x() + aMuxerDataLabel.width() / 2,
                                         ALUPoly.boundingRect().y() - arrowHOffset) // Arrow Middle point
                               << QPoint(aMuxerDataLabel.x() + aMuxerDataLabel.width() / 2 - 10,
                                         ALUPoly.boundingRect().y() - (arrowHDepth - 5)) // Arrow Left Outer point
                               << QPoint(aMuxerDataLabel.x() + aMuxerDataLabel.width() / 2 - 5,
                                         ALUPoly.boundingRect().y() - (arrowHDepth - 5)) // Arrow Left Inner point
    );

const QPolygon ABus = QPolygon(
    QVector<QPoint>()
    ...
    << QPoint(aMuxerDataLabel.right() - 15,
              MARMuxerDataLabel.bottom() - 5) // Pivot between AMUX arrow left point and MARMux Inner Bottom Edge
    << QPoint(MARMuxerDataLabel.right() + (arrowHDepth - 5),
              MARMuxerDataLabel.bottom() - 5) // MARMux Arrow Bottom Inner point
    << QPoint(MARMuxerDataLabel.right() + (arrowHDepth - 5),
              MARMuxerDataLabel.bottom() - 0) // MARMux Arrow Bottom Outer point
    << QPoint(MARMuxerDataLabel.right() + (arrowHOffset), MARMuxerDataLabel.bottom() - 10) // MARMux Arrow Middle point
    << QPoint(MARMuxerDataLabel.right() + (arrowHDepth - 5),
              MARMuxerDataLabel.bottom() - 20) // MARMux Arrow Top Outer point
    << QPoint(MARMuxerDataLabel.right() + (arrowHDepth - 5),
              MARMuxerDataLabel.bottom() - 15) // MARMux Arrow Top Inner point
    << QPoint(aMuxerDataLabel.right() - 15,
              MARMuxerDataLabel.bottom() - 15) // Pivot between MARMux arrow top and register top left foot
);
const QPolygon BBus = QPolygon(
    QVector<QPoint>()
    ...
    << QPoint(ALUUpperRightLineMidpoint + 5,
              ALUPoly.boundingRect().y() - (arrowHDepth - 5)) // ALU Arrow Right Inner point
    << QPoint(ALUUpperRightLineMidpoint + 10,
              ALUPoly.boundingRect().y() - (arrowHDepth - 5))                             // ALU Arrow Right Outer point
    << QPoint(ALUUpperRightLineMidpoint + 0, ALUPoly.boundingRect().y() - (arrowHOffset)) // ALU Arrow Middle point
    << QPoint(ALUUpperRightLineMidpoint - 10,
              ALUPoly.boundingRect().y() - (arrowHDepth - 5)) // ALU Arrow Left Outer point
    << QPoint(ALUUpperRightLineMidpoint - 5,
              ALUPoly.boundingRect().y() - (arrowHDepth - 5))            // ALU Arrow Left Inner point
    << QPoint(ALUUpperRightLineMidpoint - 5, MARMuxerDataLabel.y() + 15) // Pivot between ALU arrow and MARMux Arrow
    << QPoint(MARMuxerDataLabel.right() + (arrowHDepth - 5),
              MARMuxerDataLabel.y() + 15) // MARMux Arrow Bottom Inner point
    << QPoint(MARMuxerDataLabel.right() + (arrowHDepth - 5),
              MARMuxerDataLabel.y() + 20)                                             // MARMux Arrow Bottom Outer point
...
const QPolygon CBus = QPolygon(
    ...
    << QPoint(cMuxerLabel.x() + cMuxerLabel.width() / 2 - 5,
              MDREMuxerDataLabel.bottom() + (arrowHDepth - 5) +
                  CBusToMDREMuxLength) // Pivot between CMux foot and MDRE lower Leg
    << QPoint(MDREMuxerDataLabel.right() - 15,
              MDREMuxerDataLabel.bottom() + (arrowHDepth - 5) +
                  CBusToMDREMuxLength) // Pivot between MDRE lower leg and MDRE left arrow
    << QPoint(MDREMuxerDataLabel.right() - 15,
              MDREMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MDREMux Arrow Left Inner Point
    << QPoint(MDREMuxerDataLabel.right() - 20,
              MDREMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MDREMux Arrow Left Outer Point
    << QPoint(MDREMuxerDataLabel.right() - 10,
              MDREMuxerDataLabel.bottom() + (arrowHOffset)) // MDREMux Arrow Middle Point
    << QPoint(MDREMuxerDataLabel.right() - 0,
              MDREMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MDREMux Arrow Right Outer Point
    << QPoint(MDREMuxerDataLabel.right() - 5,
              MDREMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MDREMux Arrow Right Inner Point
    << QPoint(MDREMuxerDataLabel.right() - 5, MDREMuxerDataLabel.bottom() + (arrowHDepth - 5) + CBusToMDREMuxLength -
                                                  10) // Pivot between MDRE right arrow and MDRE upper leg
    << QPoint(cMuxerLabel.x() + cMuxerLabel.width() / 2 - 5, MDREMuxerDataLabel.bottom() + (arrowHDepth - 5) +
                                                                 CBusToMDREMuxLength -
                                                                 10) // Pivot between MDRE upper leg and leg upwards
    // Branch off to MDROdd
    << QPoint(cMuxerLabel.x() + cMuxerLabel.width() / 2 - 5,
              MDROMuxerDataLabel.bottom() + (arrowHDepth - 5) + 20 + 5) // Pivot between CMux foot and MDRE lower Leg
    << QPoint(MDROMuxerDataLabel.right() - 15, MDROMuxerDataLabel.bottom() + (arrowHDepth - 5) + 20 +
                                                   5) // Pivot between MDRE lower leg and MDRE left arrow
    << QPoint(MDROMuxerDataLabel.right() - 15,
              MDROMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MDREMux Arrow Left Inner Point
    << QPoint(MDROMuxerDataLabel.right() - 20,
              MDROMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MDREMux Arrow Left Outer Point
    << QPoint(MDROMuxerDataLabel.right() - 10,
              MDROMuxerDataLabel.bottom() + (arrowHOffset)) // MDREMux Arrow Middle Point
    << QPoint(MDROMuxerDataLabel.right() - 0,
              MDROMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MDREMux Arrow Right Outer Point
    << QPoint(MDROMuxerDataLabel.right() - 5,
              MDROMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MDREMux Arrow Right Inner Point
    << QPoint(MDROMuxerDataLabel.right() - 5, MDROMuxerDataLabel.bottom() + (arrowHDepth - 5) + 10 +
                                                  5) // Pivot between MDRE right arrow and MDRE upper leg
    << QPoint(cMuxerLabel.x() + cMuxerLabel.width() / 2 - 5,
              MDROMuxerDataLabel.bottom() + (arrowHDepth - 5) + 10 + 5) // Pivot between MDRE upper leg and leg upwards
    // Resume path to register bank's arrow
    << QPoint(cMuxerLabel.x() + cMuxerLabel.width() / 2 - 5,
              RegBank.bottom() + (arrowHDepth - 5)) // Register Arrow Left Inner Point
    << QPoint(cMuxerLabel.x() + cMuxerLabel.width() / 2 - 10,
              RegBank.bottom() + (arrowHDepth - 5)) // Register Arrow Left Outer Point
    << QPoint(cMuxerLabel.x() + cMuxerLabel.width() / 2 + 0,
              RegBank.bottom() + (arrowHOffset)) // Register Arrow Middle Point
    << QPoint(cMuxerLabel.x() + cMuxerLabel.width() / 2 + 10,
              RegBank.bottom() + (arrowHDepth - 5)) // Register Arrow Right Outer Point
    << QPoint(cMuxerLabel.x() + cMuxerLabel.width() / 2 + 5,
              RegBank.bottom() + (arrowHDepth - 5)) // Register Arrow Right Inner Point
);
const QPolygon AddrArrow = OneByteShapes::AddrArrow;
// const QPolygon DataToMDRMuxBus;
const QPolygon DataToMDROMuxBus = QPolygon(
    QVector<QPoint>()
    // Foot:
    << QPoint(MDROMuxerDataLabel.x() + 15, MDROMuxerDataLabel.bottom() + (arrowHDepth - 5) + 18 +
    ...
    << QPoint(MDROMuxerDataLabel.x() + 5, MDROMuxerDataLabel.bottom() + (arrowHDepth - 5) + 8 +
...
const QPolygon DataToMDREMuxBus = QPolygon(
    QVector<QPoint>()
    // Foot:
    << QPoint(MDREMuxerDataLabel.x() + 15, MDREMuxerDataLabel.bottom() + (arrowHDepth - 5) +
    ...
    << QPoint(MDREMuxerDataLabel.x() + 5, MDREMuxerDataLabel.bottom() + (arrowHDepth - 5) +
...
const QPolygon MDROToDataBus = QPolygon(
    QVector<QPoint>()
    ...
    << QPoint(DataBus.x() + DataBus.width() + 13,
              MDROLabel.y() + MDROLabel.height() / 2 - 10)                               // Arrow Outer Upper point
    << QPoint(DataBus.x() + DataBus.width() + 3, MDROLabel.y() + MDROLabel.height() / 2) // Arrow middle
    << QPoint(DataBus.x() + DataBus.width() + 13,
              MDROLabel.y() + MDROLabel.height() / 2 + 10)                                    // Arrow Outer Lower Point
...
const QPolygon MDREToDataBus = QPolygon(
    QVector<QPoint>()
    ...
    << QPoint(DataBus.x() + DataBus.width() + 13,
              MDRELabel.y() + MDRELabel.height() / 2 + 10) // Arrow Botton Outer point
    << QPoint(DataBus.x() + DataBus.width() + 13,
              MDRELabel.y() + MDRELabel.height() / 2 + 5)                  // Arrow Bottom Inner point
    << QPoint(MDRELabel.x(), MDRELabel.y() + MDRELabel.height() / 2 + 5)); // Foot Bottom point
const QPolygon MDREToEOMuxBus = QPolygon(
    QVector<QPoint>()
    ...
    << QPoint(EOMuxerDataLabel.x() + 5,
              MDRELabel.y() + MDRELabel.height() / 2 + 5) // Pivot between MDRE bottom and EOMux bottom
    ...
    << QPoint(EOMuxerDataLabel.x() + 15,
              MARMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MARMux Arrow Right Inner point
    << QPoint(EOMuxerDataLabel.x() + 20,
              MARMuxerDataLabel.bottom() + (arrowHDepth - 5))                         // MARMux Arrow Right Outer point
    ...
    << QPoint(EOMuxerDataLabel.x() + 5,
              MDRELabel.y() + MDRELabel.height() / 2 - 5) // Pivot between MARMux bottom and MDRE top
);
const QPolygon MDROToEOMuxBus = QPolygon(
    QVector<QPoint>()
    ...
    << QPoint(EOMuxerDataLabel.right() - 15,
              MDROLabel.y() + MDROLabel.height() / 2 + 5) // Pivot between MDRE bottom and EOMux bottom
    ...
    << QPoint(EOMuxerDataLabel.right() - 5,
              MARMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MARMux Arrow Right Inner Edge
    << QPoint(EOMuxerDataLabel.right() + 0,
              MARMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MARMux Arrow Right Outer Edge
    << QPoint(EOMuxerDataLabel.right() - 10, MARMuxerDataLabel.bottom() + (arrowHOffset)) // MARMux Arrow Middle point
    << QPoint(EOMuxerDataLabel.right() - 20,
              MARMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MARMux Arrow Left Outer Edge
    << QPoint(EOMuxerDataLabel.right() - 15,
              MARMuxerDataLabel.bottom() + (arrowHDepth - 5)) // MARMux Arrow Left Inner Edge
    << QPoint(EOMuxerDataLabel.right() - 15,
              MDROLabel.y() + MDROLabel.height() / 2 - 5) // Pivot between MARMux bottom and MDRE top
);
// const QPolygon MDRMuxOutBus;
const QPolygon MDROMuxOutBus =
    QPolygon(QVector<QPoint>() << QPoint(MDROMuxerDataLabel.x() + MDROMuxerDataLabel.width() / 2 - 5,
                                         MDROMuxerDataLabel.y()) // Foot Left point
                               << QPoint(MDROMuxerDataLabel.x() + MDROMuxerDataLabel.width() / 2 - 5,
                                         MDROMuxerDataLabel.y() - 7) // Arrow Left Inner point
                               << QPoint(MDROMuxerDataLabel.x() + MDROMuxerDataLabel.width() / 2 - 10,
                                         MDROMuxerDataLabel.y() - 7) // Arrow Left Outer point
                               << QPoint(MDROMuxerDataLabel.x() + MDROMuxerDataLabel.width() / 2,
                                         MDROMuxerDataLabel.y() - 17) // Arrow Middle point
                               << QPoint(MDROMuxerDataLabel.x() + MDROMuxerDataLabel.width() / 2 + 10,
                                         MDROMuxerDataLabel.y() - 7) // Arrow Right Outer point
                               << QPoint(MDROMuxerDataLabel.x() + MDROMuxerDataLabel.width() / 2 + 5,
                                         MDROMuxerDataLabel.y() - 7) // Arrow Right Inner point
                               << QPoint(MDROMuxerDataLabel.x() + MDROMuxerDataLabel.width() / 2 + 5,
                                         MDROMuxerDataLabel.y())); // Foot Right point

const QPolygon MDREMuxOutBus =
    QPolygon(QVector<QPoint>() << QPoint(MDREMuxerDataLabel.x() + MDREMuxerDataLabel.width() / 2 - 5,
                                         MDREMuxerDataLabel.y()) // Foot Left point
                               << QPoint(MDREMuxerDataLabel.x() + MDREMuxerDataLabel.width() / 2 - 5,
                                         MDREMuxerDataLabel.y() - 7) // Arrow Left Inner point
                               << QPoint(MDREMuxerDataLabel.x() + MDREMuxerDataLabel.width() / 2 - 10,
                                         MDREMuxerDataLabel.y() - 7) // Arrow  Outer left point
                               << QPoint(MDREMuxerDataLabel.x() + MDREMuxerDataLabel.width() / 2,
                                         MDREMuxerDataLabel.y() - 17) // Arrow Middle point
                               << QPoint(MDREMuxerDataLabel.x() + MDREMuxerDataLabel.width() / 2 + 10,
                                         MDREMuxerDataLabel.y() - 7) // Arrow Right Outer point
                               << QPoint(MDREMuxerDataLabel.x() + MDREMuxerDataLabel.width() / 2 + 5,
                                         MDREMuxerDataLabel.y() - 7) // Arrow Right Inner point
                               << QPoint(MDREMuxerDataLabel.x() + MDREMuxerDataLabel.width() / 2 + 5,
                                         MDREMuxerDataLabel.y())); // Foot Right point

const QPolygon ALUOutBus = OneByteShapes::ALUOutBus.translated(controlOffsetX, aluOffsetY);

const Arrow ALUSelect =
    Arrow(QVector<QPoint>() << QPoint(ALUPoly.boundingRect().right() - 13, ALUPoly.boundingRect().bottom() - 21),
          QVector<QLine>() << QLine(ALUPoly.boundingRect().right() - 13, ALULineEdit.y() + selectYOffset - 1,
...
const QLine CSMuxSelect = QLine(CSMuxerDataLabel.right() + arrowHOffset, CSMuxLabel.y() + selectYOffset + 1,
                                ctrlInputX - 7, CSMuxLabel.y() + selectYOffset + 1);
const QLine SBitSelect = QLine(sBitLabel.right() + arrowHOffset, sBitLabel.y() + selectYOffset, ctrlInputX - 7,
                               sBitLabel.y() + selectYOffset);
const QLine CBitSelect = QLine(cBitLabel.right() + arrowHOffset, cBitLabel.y() + selectYOffset, ctrlInputX - 7,
                               cBitLabel.y() + selectYOffset);
const QLine VBitSelect = QLine(vBitLabel.right() + arrowHOffset, vBitLabel.y() + selectYOffset, ctrlInputX - 7,
                               vBitLabel.y() + selectYOffset);
const QLine ZBitSelect = QLine(zBitLabel.right() + arrowHOffset, zBitLabel.y() + selectYOffset, ctrlInputX - 7,
                               zBitLabel.y() + selectYOffset);
const QLine NBitSelect = QLine(nBitLabel.right() + arrowHOffset, nBitLabel.y() + selectYOffset, ctrlInputX - 7,
...
const Arrow AndZOut = Arrow(
    QVector<QPoint>() << QPoint(AndZMuxLabel.x() + AndZMuxLabel.width() / 2 - arrowHOffset / 2,
                                AndZMuxLabel.top() - selectYOffset - 2)
                      << QPoint(zBitLabel.x() - 12, AndZMuxLabel.y() + AndZMuxLabel.height() / 2 - 3),
    QVector<QLine>()
        ...
        << QLine(AndZMuxLabel.right(), AndZMuxLabel.y() + AndZMuxLabel.height() / 2, zBitLabel.left() - arrowHOffset,
                 AndZMuxLabel.y() + AndZMuxLabel.height() / 2));

const QLine MemReadSelect = QLine(DataBus.right() + arrowHOffset, MemReadLabel.y() + selectYOffset, ctrlInputX - 7,
                                  MemReadLabel.y() + selectYOffset);
const QLine MemWriteSelect = QLine(DataBus.right() + arrowHOffset, MemWriteLabel.y() + selectYOffset, ctrlInputX - 7,
