...
namespace {
std::shared_ptr<BuiltinBlueprint> add_2x1(CircuitProject &f, BlueprintLibrary &library, schematic::BlueprintGroupID gid,
                                          const std::string &name) {
  auto ret = std::make_shared<BuiltinBlueprint>();
  ret->size = schematic::Size{2, 3};
  ret->pins = {
...
u32 add_2x1_group(CircuitProject &f, BlueprintLibrary &library, const std::string &name, const std::string &path,
                  BuiltinBlueprint::Type type) {
  ...
  auto gate2 = add_2x1(f, library, group_id, name + "2x1");
  gate2->type = type;
  gate2->image = fpath;

  return 1;
}
u32 add_and_blueprints(CircuitProject &f, BlueprintLibrary &library) {
  return add_2x1_group(f, library, "AND", ":/and", BuiltinBlueprint::Type::AND);
}

u32 add_or_blueprints(CircuitProject &f, BlueprintLibrary &library) {
  return add_2x1_group(f, library, "OR", ":/or", BuiltinBlueprint::Type::OR);
}

u32 add_nand_blueprints(CircuitProject &f, BlueprintLibrary &library) {
  return add_2x1_group(f, library, "NAND", ":/nand", BuiltinBlueprint::Type::NAND);
}

u32 add_nor_blueprints(CircuitProject &f, BlueprintLibrary &library) {
  return add_2x1_group(f, library, "NOR", ":/nor", BuiltinBlueprint::Type::NOR);
}

u32 add_xor_blueprints(CircuitProject &f, BlueprintLibrary &library) {
  return add_2x1_group(f, library, "XOR", ":/xor", BuiltinBlueprint::Type::XOR);
}
...
u32 CircuitProject::add_builtin_blueprints() {
  u32 ret = 0;
  ret += add_and_blueprints(*this, *_library);
  ret += add_or_blueprints(*this, *_library);
  ret += add_nand_blueprints(*this, *_library);
  ret += add_nor_blueprints(*this, *_library);
  ret += add_xor_blueprints(*this, *_library);
  return ret;
}

void CircuitProject::add_test_data() {
  auto and_blueprint = _library->get_blueprint("AND2x1");
  auto and_id = _schematic->place_component(and_blueprint, schematic::Point{6, 4}, Direction::Right);
  ...
  auto or_id = _schematic->place_component(or_blueprint, schematic::Point{2, 6}, Direction::Right);
  ...
  auto xor_id = _schematic->place_component(xor_blueprint, schematic::Point{2, 2}, Direction::Right);
