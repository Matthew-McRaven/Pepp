...
SelectMode policyToMode(symbol::TraversalPolicy policy) {
  ...
  case symbol::TraversalPolicy::kChildren:
    [[fallthrough]];
  case symbol::TraversalPolicy::kSiblings:
    [[fallthrough]];
  case symbol::TraversalPolicy::kWholeTree:
    return static_cast<SelectMode>(kSelf | kChildren);
...
void selectByNameImpl(QList<QSharedPointer<symbol::Entry>> &list,
                      QSharedPointer<symbol::Table> table, const QString &name,
                      SelectMode mode) {
  if (mode & kChildren) {
  ...
  if (mode & kSelf) {
    ...
    if (maybeSymbol)
      list.push_back(maybeSymbol.value());
  }
}
QList<QSharedPointer<symbol::Entry>>
symbol::selectByName(QSharedPointer<Table> table, const QString &name,
                     TraversalPolicy policy) {
  QList<QSharedPointer<symbol::Entry>> ret;
  selectByNameImpl(ret, policyToTargetTable(policy, table), name,
                   policyToMode(policy));
  return ret;
}

void existsImpl(bool &exists, QSharedPointer<symbol::Table> table,
...
bool symbol::exists(QSharedPointer<Table> table, const QString &name,
                    TraversalPolicy policy) {
  bool ret = false;
  existsImpl(ret, policyToTargetTable(policy, table), name,
             policyToMode(policy));
  return ret;
}

void adjustOffsetImpl(QSharedPointer<symbol::Table> table, quint64 offset,
                      quint64 threshold, SelectMode mode) {
  if (mode & kChildren) {
    for (auto &child : table->children()) {
...
void symbol::adjustOffset(QSharedPointer<Table> table, quint64 offset,
                          quint64 threshold, TraversalPolicy policy) {
  adjustOffsetImpl(policyToTargetTable(policy, table), offset, threshold,
                   policyToMode(policy));
}

void enumerateImpl(QList<QSharedPointer<symbol::Entry>> &list,
                   QSharedPointer<symbol::Table> table, SelectMode mode) {
  if (mode & kChildren) {
    for (auto &child : table->children()) {
...
QList<QSharedPointer<symbol::Entry>>
symbol::enumerate(QSharedPointer<Table> table, TraversalPolicy policy) {
  QList<QSharedPointer<symbol::Entry>> ret;
  enumerateImpl(ret, policyToTargetTable(policy, table), policyToMode(policy));
  return ret;
}

QString symbol::tableListing(QSharedPointer<Table> table, quint8 maxBytes,
                             TraversalPolicy policy) {
