...
TEST_CASE("Pepp AST conversion, failing", "[parse]") {
  ...
  auto [name, input, errors] = GENERATE(table<std::string, QString, QList<Error>>({
      ...
      {"nonunary store: I is bad addressing mode-1 byte", u"stba 0xfc16,i"_qs,
       QList<Error>{makeFatal(0, E::illegalAddrMode)}},
      {"nonunary store: I is bad addressing mode-2 byte", u"stwa 0xfc16,i"_qs,
       QList<Error>{makeFatal(0, E::illegalAddrMode)}},
      ...
      {".SECTION: max 2 argument", u".SECTION 0x0, 0x1, 0x2"_qs,
       QList<Error>{makeFatal(0, E::expectNMArguments.arg(1, 2))}},
      // - no symbol
      {".SECTION: no symbol", u"ret: .SECTION \"data\""_qs,
