...
void injectFakeSCallMacros(QSharedPointer<macro::Registry> registry)
{
    ...
    for (auto &macro : nonunary)
        registry->registerMacro(macro::types::Core,
                                QSharedPointer<macro::Parsed>::create(macro,
                                                                      2,
                                                                      "LDWA 0,i\nSCALL $1, $2",
                                                                      "pep/10"));
}
        ...
        for (auto &fig : book->figures()) {
            ...
            DYNAMIC_SECTION(chapter.toStdString() << "." << figName.toStdString())
            {
                ...
                if (isFullOS) {
                    ...
                    mergeMap[0].segs = {};
                    auto uMem = obj::MemoryRegion{.r = 1,
                                                  .w = 1,
                                                  .minOffset = 0,
                                                  .maxOffset = 0xfaf9};
                    ...
                    // Carveout for MMIO
                    mergeMap[2].segs = {};
                    auto mmio = obj::MemoryRegion{.r = 1,
                                                  .w = 1,
                                                  .minOffset = 0xfffa,
                                                  .maxOffset = 0xffff};
                ...
                REQUIRE_NOTHROW(
                    [&sys, &elf]() { sys = targets::pep10::isa::systemFromElf(*elf, true); }());
