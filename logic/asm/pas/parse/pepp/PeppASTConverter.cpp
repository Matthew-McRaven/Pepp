...
QString errorFromWordString(QSharedPointer<value::Base> arg) {
    if (dynamic_cast<value::Hexadecimal *>(arg.data()) != nullptr)
        return pas::errors::pepp::hexTooBig2;
    else if (dynamic_cast<value::ShortString *>(arg.data()) != nullptr)
...
std::any parse::PeppASTConverter::visitDirectiveLine(PeppParser::DirectiveLineContext *context)
{
    ...
    static QMap<QString, convert_fn> converters = {
        {u"ALIGN"_qs,   &PeppASTConverter::align},
        {u"ASCII"_qs,   &PeppASTConverter::ascii},
        {u"BLOCK"_qs,   &PeppASTConverter::block},
        {u"BURN"_qs,    &PeppASTConverter::burn},
        {u"BYTE"_qs,    &PeppASTConverter::byte},
        {u"END"_qs,     &PeppASTConverter::end},
        {u"EQUATE"_qs,  &PeppASTConverter::equate},
        {u"EXPORT"_qs,  &PeppASTConverter::_export},
        {u"IMPORT"_qs,  &PeppASTConverter::import},
        {u"INPUT"_qs,   &PeppASTConverter::input},
        {u"OUTPUT"_qs,  &PeppASTConverter::output},
        {u"ORG"_qs,     &PeppASTConverter::org},
        {u"SCALL"_qs,   &PeppASTConverter::scall},
        {u"SECTION"_qs, &PeppASTConverter::section},
        {u"USCALL"_qs,  &PeppASTConverter::uscall},
        {u"WORD"_qs,    &PeppASTConverter::word},
    };
...
std::any parse::PeppASTConverter::visitArgument(PeppParser::ArgumentContext *context)
{
    ...
    if(auto tok = context->CHARACTER(); tok) {
        ...
        if (auto length = bits::escapedStringLength(asQString); length <= 2)
            ret = QSharedPointer<value::ShortString>::create(
                asQString, length, bits::Order::BigEndian);
        else
            ret = QSharedPointer<value::LongString>::create(
...
void parse::PeppASTConverter::ascii(QSharedPointer<pas::ast::Node> node, PeppParser::DirectiveLineContext *context)
{
    ...
    auto arg = _lineInfo.arguments.at(0);
    if (!arg->isText())
        addError(node, {.severity = S::Fatal, .message = EP::dotRequiresString.arg(u".ASCII"_qs)});
    else
        node->set(generic::Argument{.value = arg});
}
...
void parse::PeppASTConverter::block(QSharedPointer<pas::ast::Node> node, PeppParser::DirectiveLineContext *context)
{
    ...
    if (!(arg->isFixedSize() && arg->isNumeric()) || arg->isText())
        ...
        addError(node, {.severity = S::Fatal, .message = EP::decUnsigned2});
    else if (arg->requiredBytes() > 2)
        addError(node, {.severity = S::Fatal, .message = errorFromWordString(arg)});
    else
        node->set(generic::Argument{.value = arg});
...
void parse::PeppASTConverter::byte(QSharedPointer<pas::ast::Node> node, PeppParser::DirectiveLineContext *context)
{
    ...
    if (arg->isText() && arg->size() > 1)
        addError(node, {.severity = S::Fatal, .message = EP::strTooLong1});
...
void parse::PeppASTConverter::equate(QSharedPointer<pas::ast::Node> node, PeppParser::DirectiveLineContext *context)
{
    ...
    if (arg->isText() && arg->size() > 2)
        addError(node, {.severity = S::Fatal, .message = EP::strTooLong2});
    else if (!(arg->isFixedSize() && arg->isNumeric()))
        addError(node,{.severity = S::Fatal, .message = EP::expectedNumeric});
    else if (arg->requiredBytes() > 2)
        addError(node, {.severity = S::Fatal, .message = errorFromWordString(arg)});
...
void parse::PeppASTConverter::section(QSharedPointer<pas::ast::Node> node, PeppParser::DirectiveLineContext *context)
{
    ...
    if (!arg->isText())
        ...
        if (!flagArg->isText())
            addError(node, {.severity = S::Fatal, .message = EP::sectionFlagsString});
        else
            node->set(generic::ArgumentList{.value = {arg, flagArg}});
    } else
        node->set(generic::Argument{.value = arg});
...
void parse::PeppASTConverter::io_scall_helper(QSharedPointer<pas::ast::Node> node, PeppParser::DirectiveLineContext *context, QString name)
{
    if(_lineInfo.arguments.size() != 1) {
        addError(node, {.severity = S::Fatal, .message = EP::expectNArguments.arg(1)});
        return;
