...
export class Book implements IBook {
  ...
  async figures(): Promise<Array<IFigure>> {
    ...
    rows.forEach((row) => names.push(`${row.chapter_num}.${row.figure_name}`));

    const figures = await Promise.all(names.map(async (name) => this.findFigure(name)));
    return figures.filter(notEmpty);
  }
  ...
  async findFigure(maybeCombined: string, figure?: string): Promise<IFigure | null> {
    ...
    const row = await this.#db.get(
      'SELECT id, arch, kind, default_os, is_os FROM table_of_figures WHERE (chapter_num = ? and figure_name = ?)',
      [chapter, figure],
    );
    ...
    if (typeof isOS === 'number') {
      const osRow = await this.#db.get(
        'SELECT chapter_num, figure_name, kind FROM table_of_figures WHERE id = ?',
        osID,
      );
