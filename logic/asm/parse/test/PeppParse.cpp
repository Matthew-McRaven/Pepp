...
TEST_CASE("Pepp directive parsing", "[parse]") {
    ...
    auto [text, type] = GENERATE(table<std::string, std::string>({
        ...
        {".SECTIOn asdx, \"RWX\"","(prog (stat (directive .SECTIOn (argument_list (argument asdx) , (argument \"RWX\")))) <EOF>)"},
        {"symbol:.ARGL 1, 'a', ident, $1, 0x20;comment",
         "(prog (stat (symbol symbol:) (directive .ARGL (argument_list (argument 1) , (argument 'a') , (argument ident) , (argument $1) , (argument 0x20))) ;comment) <EOF>)"
        },
    }));
    single_line(text, type);
}
...
TEST_CASE("Pepp macro parsing", "[parse]") {
    ...
    auto [text, type] = GENERATE(table<std::string, std::string>({
        ...
        {"$1:@hi $1,2","(prog (stat (symbol $1:) (invoke_macro @hi (argument_list (argument $1) , (argument 2)))) <EOF>)"},
        {"@hi 10,d;comment","(prog (stat (invoke_macro @hi (argument_list (argument 10) , (argument d))) ;comment) <EOF>)"},
    }));
    single_line(text, type);
}
