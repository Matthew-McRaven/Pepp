...
Napi::Value bind::ELFWriter::write_string(const Napi::CallbackInfo &info) {
  ...
  if (const auto string_cache_it = active_cache.find(string_value); string_cache_it == active_cache.end()) {

    // See if the section already exists
    auto section_it = std::find_if(elf->sections.begin(),
                                   elf->sections.end(),
                                   [&section_name](const auto &item) { return item->get_name() == section_name; });

    // Either create or reference existing section.
    ELFIO::section *ptr = nullptr;
    if (section_it == elf->sections.end()) {
      ptr = elf->sections.add(section_name);
      ptr->set_type(ELFIO::SHT_STRTAB);
    } else {
      ptr = (*section_it).get();
    }

    // Add the string value to the section
    auto str_accessor = ELFIO::string_section_accessor(ptr);
    active_cache[string_value] = str_accessor.add_string(string_value);
    // Skip cache lookup since we already have cached value.
    return Napi::BigInt::New(env, static_cast<uint64_t>(active_cache[string_value]));
  } else
    return Napi::BigInt::New(env, static_cast<uint64_t>(string_cache_it->second));
}
