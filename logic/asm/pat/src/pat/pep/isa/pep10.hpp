...
struct Pep10ISA {
  ...
  using AddressingMode = detail::AddressingMode;
  using InstructionType = detail::InstructionType;
  using Instruction = detail::Instruction;
  using Opcode = detail::Opcode;

  static Mnemonic defaultMnemonic();
  static AddressingMode defaultAddressingMode();
  static quint8 opcode(const Mnemonic &instr);
  static quint8 opcode(const Mnemonic &inst, AddressingMode addr);
  static AddressingMode parseAddressingMode(const QString &addr);
  static Mnemonic parseMnemonic(const QString &mnemonic);
  static QString string(Mnemonic);
  static QString string(AddressingMode);
  // SCALL is a non-unary mnemonic, but a unary opcode;
  static bool isMnemonicUnary(Mnemonic);
  static bool isMnemonicUnary(quint8);
  static bool isOpcodeUnary(Mnemonic);
  static bool isOpcodeUnary(quint8);
  static bool isStore(Mnemonic);
  static bool isStore(quint8);

  static bool isUType(Mnemonic mnemonic);
  static bool isRType(Mnemonic mnemonic);
  static bool isAType(Mnemonic mnemonic);
  static bool isValidATypeAddressingMode(AddressingMode mode);
  static bool isAAAType(Mnemonic mnemonic);
  static bool isValidAAATypeAddressingMode(AddressingMode mode);
  static bool isRAAAType(Mnemonic mnemonic);
  static bool isValidRAAATypeAddressingMode(AddressingMode mode);

  constexpr static std::array<Opcode, 256> opcodeLUT = detail::initOpcodes();
};
