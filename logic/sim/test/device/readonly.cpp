...
auto desc_rw = api2::device::Descriptor {
    .id = 0, .compatible = nullptr, .baseName = "dev_rw", .fullName = "/dev_rw"
};
...
TEST_CASE("ReadOnly storage in-bounds access, v2", "[sim][memory][throws]") {
    ...
    auto span = api2::memory::AddressSpan<quint8>{
        .minOffset = 0x10, .maxOffset = 255
    };
    ...
    // but does not error when hardfail is false
    reg = 0xFEEDBEEF;
    REQUIRE_THROWS_AS(dev_ro_throws.write(0x10, {truth, length}, op_std),
                      api2::memory::Error<quint8>);
    compare_ne(truth, tmp, length);
