...
protected:
  ...
  // undo of writes.
  QSharedPointer<const Event> append_event(publisher_id_t publisher,
                                           val_size_t value);
  ...
  QSharedPointer<const Event> revert_event(publisher_id_t publisher,
                                           size_t time);
  ...
  QSharedPointer<const Event>
  next_event(QSharedPointer<const Event> event) const;
  ...
  // "skip" mechanism allows for deeply nested reverts.
  QSharedPointer<const Event>
  previous_event(QSharedPointer<const Event> event) const;
...
public:
  ...
  class Endpoint {
  public:
    Endpoint(QSharedPointer<const Event> event, publisher_id_t id,
...
protected:
  struct Event {
    ...
    Event(val_size_t value, publisher_id_t publisher, bool empty,
...
Channel<offset_t, val_size_t>::append_event(publisher_id_t publisher,
                                            val_size_t value) {
  tail->next_node = QSharedPointer<Event>::create(value, publisher, false, tail,
                                                  tail->displacement + 1);
  tail = tail->next_node;
  return tail;
}
...
template <typename offset_t, typename val_size_t>
QSharedPointer<const typename Channel<offset_t, val_size_t>::Event>
Channel<offset_t, val_size_t>::revert_event(publisher_id_t publisher,
                                            size_t time) {
  QSharedPointer<Event> fixup = {}, fixup_next = {},
                        ptr = mutable_event_at(time);
...
template <typename offset_t, typename val_size_t>
QSharedPointer<const typename Channel<offset_t, val_size_t>::Event>
Channel<offset_t, val_size_t>::next_event(
    QSharedPointer<const Event> event) const {
...
template <typename offset_t, typename val_size_t>
QSharedPointer<const typename Channel<offset_t, val_size_t>::Event>
Channel<offset_t, val_size_t>::previous_event(
    QSharedPointer<const Event> event) const {
  // If there is no subsequent event, then return the current event.
...
template <typename offset_t, typename val_size_t>
Channel<offset_t, val_size_t>::Channel(val_size_t default_value) {
  this->head = this->tail =
      QSharedPointer<Event>::create(default_value, 0, false, nullptr, 0);
}
...
Channel<offset_t, val_size_t>::new_endpoint() {
  return QSharedPointer<Endpoint>::create(tail, this->next_id++,
                                          this->sharedFromThis());
}
...
template <typename offset_t, typename val_size_t>
Channel<offset_t, val_size_t>::Endpoint::Endpoint(
    QSharedPointer<const Event> event, publisher_id_t id,
    QSharedPointer<Channel> channel)
...
template <typename offset_t, typename val_size_t>
std::optional<val_size_t>
Channel<offset_t, val_size_t>::Endpoint::current_value() const {
...
template <typename offset_t, typename val_size_t>
val_size_t Channel<offset_t, val_size_t>::Endpoint::set_to_head() {
...
template <typename offset_t, typename val_size_t>
val_size_t Channel<offset_t, val_size_t>::Endpoint::set_to_tail() {
...
template <typename offset_t, typename val_size_t>
std::optional<val_size_t>
Channel<offset_t, val_size_t>::Endpoint::next_value() const {
...
template <typename offset_t, typename val_size_t>
void Channel<offset_t, val_size_t>::Endpoint::append_value(
    val_size_t new_value) {
...
template <typename offset_t, typename val_size_t>
Channel<offset_t, val_size_t>::Event::Event(val_size_t value,
                                            publisher_id_t publisher,
                                            bool empty,
                                            QSharedPointer<Event> prev_node,
                                            size_t displacement)
