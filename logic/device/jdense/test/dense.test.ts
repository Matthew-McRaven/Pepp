...
describe('JDense', () => {
  ...
  it('handles device identification', () => {
    const mem = new JDense(devicePOD, 0n, 0x10n, 0);
  ...
  it('respects memory bounds', () => {
    ...
    expect(() => { mem.resize(0x20); }).not.toThrow();
  ...
  it('performs multi-byte wrapped reads/writes', () => {
    const mem = new JDense(devicePOD, 0n, 0x2n, 0);
    mem.setTraceBuffer(tb);
  ...
  it('participates in undo', () => {
    const mem = new JDense(devicePOD, 0n, 0x10n, 0x02);
