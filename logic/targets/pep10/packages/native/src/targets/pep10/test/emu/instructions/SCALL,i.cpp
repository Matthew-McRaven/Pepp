...
TEST_CASE("Instruction: SCALL,i", "[isa::pep10]") {
  ...
  SECTION("SCALL,i") {
    ...
    std::vector<uint8_t> program = {0x30};
    for (uint8_t start_stat = 0; start_stat + 1 < 0b1'0000; start_stat++) {
      ...
      CHECK((int) machine->get_memory(0xFEED - 1).value() == 0x30);
      ...
      CHECK(machine->read_register(isa::pep10::Register::IS) == 0x30);
    }
  }
}
