...
TEST_CASE("Instruction: BRGE,i", "[isa::pep10]") {
    ...
    SECTION("BRGE, i") {
        ...
        for (uint8_t start_stat = 0; static_cast<uint8_t>(start_stat) + 1 < 0b1'0000; start_stat++) {
            for (uint16_t opspec = 0; static_cast<uint32_t>(opspec) + 1 < 0x1'0000; opspec++) {
                ...
                std::vector<uint8_t> program = {0x26, static_cast<uint8_t>((opspec >> 8) & 0xff),
                ...
                CHECK(machine->read_register(isa::pep10::Register::IS) == 0x26);
