...
TEST_CASE("Instruction: LDBA,i", "[isa::pep10]") {
  ...
  // RTL: A[8 : 15] ← byte Oprnd ; N ← 0 , A ← r[8 : 15] = 0
  SECTION("LDBA, i") {
    ...
    for (uint8_t vc = 0; static_cast<uint8_t>(vc) + 1 < 0b1'00; vc++) {
      for (uint16_t opspec = 0; static_cast<uint32_t>(opspec) + 1 < 0x1'0000; opspec++) {
        ...
        std::vector<uint8_t> program = {0x50, static_cast<uint8_t>((opspec >> 8) & 0xff),
        ...
        CHECK(machine->read_register(isa::pep10::Register::IS) == 0x50);
        // OS loaded the Mem[0x0001-0x0002].
        CHECK(machine->read_register(isa::pep10::Register::OS) == opspec);
        // Check that target register was had operand loaded.
        CHECK(machine->read_register(isa::pep10::Register::A) == (opspec & 0xFF));
        // Check that target status bits match RTL.
        CHECK(machine->read_csr(isa::pep10::CSR::N) == 0);
        CHECK(machine->read_csr(isa::pep10::CSR::Z) == (((opspec & 0xFF) == 0) ? 1 : 0));
      }
    }
  }
}
