...
TEST_CASE("Instruction: ROLA", "[isa::pep10]") {
    ...
    std::vector<uint8_t> program = {0x18};
    // RTL: A ← r[0] , A[0 : 14] ← A[1 : 15] , A[15] ← C
    SECTION("ROLA") {
        ...
        for (uint8_t nzvc = 0; static_cast<uint8_t>(nzvc) + 1 < 0b1'0000; nzvc++) {
            for (uint16_t A = 0; static_cast<uint32_t>(A) + 1 < 0x1'0000; A++) {
                ...
                CHECK(machine->read_register(isa::pep10::Register::IS) == 0x18);
                auto new_A = machine->read_register(isa::pep10::Register::A);
                // A is the starting A, shifted left by 1 bit. The carry is shifted into A[15].
                CHECK(new_A == (static_cast<uint16_t>(A << 1) | start_c));
