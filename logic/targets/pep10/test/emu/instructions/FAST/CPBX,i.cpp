...
TEST_CASE("Instruction: CPBX,i", "[isa::pep10]") {
    ...
    // RTL: Y ← X[8 : 15] − byte Oprnd ; N ← Y < 0 , Z ← Y = 0 , V ← 0 , C ← 0
    SECTION("CPBX, i") {
        ...
        for (auto target_reg_val : target_reg_values) {
            for (uint16_t opspec = 0; static_cast<uint32_t>(opspec) + 1 < 0x1'0000; opspec++) {
                ...
                std::vector<uint8_t> program = {0x98, static_cast<uint8_t>((opspec >> 8) & 0xff),
                ...
                CHECK(machine->read_register(isa::pep10::Register::IS) == 0x98);
