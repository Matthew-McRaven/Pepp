...
TEST_CASE("Instruction: MOVTA", "[isa::pep10]") {
    ...
    for (uint8_t start_stat = 0; start_stat <= 0b1111; start_stat++) {
        ...
        DYNAMIC_SECTION(fmt::format("MOVTA: NZVC={:b}", start_stat)) {
            ...
            machine->write_register(isa::pep10::Register::TR, 0xDEAD);
            ...
            CHECK(machine->read_register(isa::pep10::Register::TR) == 0xDEAD);
            ...
            CHECK(machine->read_register(isa::pep10::Register::IS) == 0x6);
            // A took on the value of T.
