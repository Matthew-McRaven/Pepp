...
TEST_CASE("Instruction: ASRA", "[isa::pep10]") {
    ...
    std::vector<uint8_t> program = {0x16};
    ...
    SECTION("ASRA") {
        ...
        for (uint8_t v = 0; static_cast<uint8_t>(v) + 1 < 0b10; v++) {
            for (uint16_t A = 0; static_cast<uint32_t>(A) + 1 < 0x1'0000; A++) {
                ...
                CHECK(machine->read_register(isa::pep10::Register::IS) == 0x16);
                auto new_A = machine->read_register(isa::pep10::Register::A);
                // A is the starting A, shifted right by 1 bit.
                // Since using unsigned shift, must manually set high order position to 1 if necessary.
                CHECK(new_A == (((A & 0x8000) ? 1 << 15 : 0) | static_cast<uint16_t>(A >> 1)));
