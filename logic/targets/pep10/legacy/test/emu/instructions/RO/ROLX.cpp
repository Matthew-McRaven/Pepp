...
TEST_CASE("Instruction: ROLX", "[isa::pep10]") {
    ...
    std::vector<uint8_t> program = {0x19};
    // RTL: X ← r[0] , X[0 : 14] ← X[1 : 15] , X[15] ← C
    SECTION("ROLX") {
        ...
        for (uint8_t nzvc = 0; static_cast<uint8_t>(nzvc) + 1 < 0b1'0000; nzvc++) {
            for (uint16_t X = 0; static_cast<uint32_t>(X) + 1 < 0x1'0000; X++) {
                ...
                CHECK(machine->read_register(isa::pep10::Register::IS) == 0x19);
                auto new_X = machine->read_register(isa::pep10::Register::X);
                // X is the starting X, shifted left by 1 bit. The carry is shifted into X[15].
                CHECK(new_X == (static_cast<uint16_t>(X << 1) | start_c));
