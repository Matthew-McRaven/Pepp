...
TEST_CASE("Instruction: CPWX,i", "[isa::pep10]") {
    ...
    // RTL: Y ← X - Oprnd ; N ← Y < 0 , Z ← Y = 0 , V ← {overflow} , C ← {carry}; N ← N ⊕ V
    SECTION("CPWX, i") {
        ...
        for (auto target_reg_val : target_reg_values) {
            for (uint16_t opspec = 0; static_cast<uint32_t>(opspec) + 1 < 0x1'0000; opspec++) {
                ...
                std::vector<uint8_t> program = {0x88, static_cast<uint8_t>((opspec >> 8) & 0xff),
                ...
                CHECK(machine->read_register(isa::pep10::Register::IS) == 0x88);
                ...
                auto difference = static_cast<uint16_t>(target_reg_val + (~opspec + 1));
                ...
                auto output_sign_match = ~(difference ^ target_reg_val) & 0x8000;
                ...
                // Check that the more complex xor behavior works correctly.
                auto negative = (difference & 0x8000 ? 1 : 0);
                CHECK(machine->read_csr(isa::pep10::CSR::N) == (negative ^ signed_overflow));
            }
        }
    }
}
