...
TEST_CASE("Instruction: NEGA", "[isa::pep10]") {
    ...
    std::vector<uint8_t> program = {0x12};
    ...
    SECTION("NEGA") {
        ...
        for (uint8_t c = 0; c <= 0b1; c++) {
            for (uint16_t A = 0; static_cast<uint32_t>(A) + 1 < 0x1'0000; A++) {
                ...
                CHECK(machine->read_register(isa::pep10::Register::IS) == 0x12);
                ...
                CHECK(machine->read_csr(isa::pep10::CSR::V) == ((new_A == 0x8000) ? 1 : 0));
            }
        }
    }
}
