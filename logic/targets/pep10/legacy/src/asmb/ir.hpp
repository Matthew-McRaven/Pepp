...
class unary_instruction : public ir::linear_line<uint16_t> {
  public:
    unary_instruction();
    ...
    std::shared_ptr<ir::linear_line<uint16_t>> clone() const override;

    ir::ByteType bytes_type() const override;
    ...
    friend void swap(unary_instruction &first, unary_instruction &second) {
        using std::swap;
        swap(static_cast<ir::linear_line<uint16_t> &>(first), static_cast<ir::linear_line<uint16_t> &>(second));
...
class nonunary_instruction : public ir::linear_line<uint16_t> {
  public:
    ...
    std::shared_ptr<ir::lir_argument<uint16_t>> argument = nullptr;
};
