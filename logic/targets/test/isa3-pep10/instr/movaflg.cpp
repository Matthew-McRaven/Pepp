...
class ISA3Pep10_MOVAFLG : public QObject {
  Q_OBJECT
private slots:
  void i() {
    auto [mem, cpu] = make();
    ...
    for (uint16_t flg = 0; static_cast<uint32_t>(flg) + 1 < 0b1'0000; flg++) {
      ...
      auto program = std::array<quint8, 1>{
          (quint8) isa::Pep10::Mnemonic::MOVAFLG};

      cpu->regs()->clear(0);
      cpu->csrs()->clear(0);

      QVERIFY_THROWS_NO_EXCEPTION(targets::pep10::isa::writeRegister(cpu->regs(), isa::Pep10::Register::A, flg, rw));
      ...
      QCOMPARE(rreg(isa::Pep10::Register::IS), (quint8)isa::Pep10::Mnemonic::MOVAFLG);
