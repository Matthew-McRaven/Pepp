...
class ISA3Pep10_STWX : public QObject {
  Q_OBJECT
private slots:
  void i() {
    ...
    auto program = std::array<quint8, 3>{
        1+(quint8) isa::Pep10::Mnemonic::STWX,static_cast<uint8_t>((opspec >> 8) & 0xff),
    ...
    QCOMPARE(rreg(isa::Pep10::Register::IS), 1+(quint8)isa::Pep10::Mnemonic::STWX);
    ...
    QVERIFY(mem->read(opspec,{reinterpret_cast<quint8 *>(&tmp), 2}, rw).completed);
