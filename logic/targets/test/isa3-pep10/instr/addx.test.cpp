...
class ISA3Pep10_ADDX : public QObject {
  Q_OBJECT
private slots:
  void i() {
    ...
    for (uint16_t opspec = 0; static_cast<uint32_t>(opspec) + 1 < 0x1'0000;
      ...
      auto program = std::array<quint8, 3>{
          0xA8, static_cast<uint8_t>((opspec >> 8) & 0xff),
      ...
      QCOMPARE(rreg(isa::Pep10::Register::IS), 0xA8);
...
#include "addx.test.moc"

QTEST_MAIN(ISA3Pep10_ADDX)
