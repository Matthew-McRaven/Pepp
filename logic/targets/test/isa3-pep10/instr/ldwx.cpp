...
class ISA3Pep10_LDWX : public QObject {
  Q_OBJECT
private slots:
  void i() {
    auto [mem, cpu] = make();
    ...
    for (uint16_t opspec = 0; static_cast<uint32_t>(opspec) + 1 < 0x1'0000;
      ...
      auto program = std::array<quint8, 3>{
          (quint8) isa::Pep10::Mnemonic::LDWX, static_cast<uint8_t>((opspec >> 8) & 0xff),
      ...
      QCOMPARE(rreg(isa::Pep10::Register::IS), (quint8)isa::Pep10::Mnemonic::LDWX);
