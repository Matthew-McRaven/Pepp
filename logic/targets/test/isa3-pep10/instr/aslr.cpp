...
template<isa::Pep10::Register target_reg>
void inner(isa::Pep10::Mnemonic op)
{
    ...
    DYNAMIC_SECTION("with initial value " << init_reg)
    {
        ...
        CHECK(csr(cpu, isa::Pep10::CSR::V) == ((top_2_bits % 2) ? 1 : 0));
        // Carry out if high order bit was non-zero
        CHECK(csr(cpu, isa::Pep10::CSR::C) == ((init_reg & 0x8000) ? 1 : 0));
    }
}
TEST_CASE("ASLA", "[pep10][isa]")
{
    using Register = isa::Pep10::Register;
    inner<Register::A>(isa::Pep10::Mnemonic::ASLA);
}
TEST_CASE("ASLX", "[pep10][isa]")
{
    using Register = isa::Pep10::Register;
    inner<Register::X>(isa::Pep10::Mnemonic::ASLX);
}
