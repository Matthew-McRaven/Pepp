...
TEST_CASE("Instruction: MOVFLGA", "[isa::pep10]") {
  ...
  for (uint8_t start_stat = 0; start_stat <= 0b1111; start_stat++) {
    // RTL: A[8 : 11] ← 0 , A[12 : 15] ← NZVC
    DYNAMIC_SECTION(fmt::format("MOVFLGA: NZVC={:b}", start_stat)) {
      ...
      std::vector<uint8_t> program = {0x04};
      ...
      CHECK(machine->read_register(isa::pep10::Register::IS) == 0x4);
      // High order bits of A were 0'ed, and [12:15] took on NZVC.
      CHECK(machine->read_register(isa::pep10::Register::A) == start_stat);
    }
  }
}
