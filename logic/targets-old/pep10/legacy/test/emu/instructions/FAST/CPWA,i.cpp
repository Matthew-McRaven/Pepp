...
TEST_CASE("Instruction: CPWA,i", "[isa::pep10]") {
    ...
    // RTL: Y ← A - Oprnd ; N ← Y < 0 , Z ← Y = 0 , V ← {overflow} , C ← {carry}; N ← N ⊕ V
    SECTION("CPWA, i") {
        ...
        for (auto target_reg_val : target_reg_values) {
            for (uint16_t opspec = 0; static_cast<uint32_t>(opspec) + 1 < 0x1'0000; opspec++) {
                ...
                std::vector<uint8_t> program = {0x80, static_cast<uint8_t>((opspec >> 8) & 0xff),
                ...
                CHECK(machine->read_register(isa::pep10::Register::IS) == 0x80);
