...
void obj::addMMIBuffer(ELFIO::elfio &elf, const ELFIO::segment *bufferableSeg) {
  ...
  bits::memcpy_endian(descSpan.first(2), bits::Order::BigEndian, bufferableSeg->get_index());
...
QList<obj::MMIBuffer> obj::getMMIBuffers(const ELFIO::elfio &elf) {
  ...
  for (int it = 0; it < noteAc.get_notes_num(); it++) {
    // Check that note exists and is from me.
    if (!noteAc.get_note(it, type, name, desc, descSize)) continue;
    else if (name != "pepp.mmios") continue;
    else if (type != 0x10) continue;

    char *port = desc + 2;
    auto descSpan = bits::span<const quint8>{reinterpret_cast<const quint8 *>(desc), descSize};
    // Must use copy helper to maintain stable bit order between hosts.
    auto segIdx = bits::memcpy_endian<ELFIO::Elf_Half>(descSpan.first(2), bits::Order::BigEndian);
    if (elf.segments.size() - 1 < segIdx) continue;
    ret.push_back(obj::MMIBuffer{.seg = elf.segments[segIdx], .portName = port});
  }
  return ret;
}
...
void obj::setBootFlagAddress(ELFIO::elfio &elf, QString name) {
  ...
  for (int i = 0; i < elf.sections.size(); i++) {
    ...
    for (int j = 0; j < symTabAc.get_symbols_num(); j++) {
      // Skip to next iteration if it does not name a valid symbol.
      if (!symTabAc.get_symbol(j, _name, value, size, bind, symbolType, index, other)) continue;
      if (nameStd == _name) goto found;
    }
  }
  return;
found:
  ...
  bits::memcpy_endian(descSpan.first(bootFlagAddrSize), bits::Order::BigEndian,
                      {reinterpret_cast<const quint8 *>(&value), sizeof(value)}, bits::hostOrder());
  bits::memcpy_endian(descSpan.subspan(bootFlagAddrSize, bootFlagSizeSize), bits::Order::BigEndian,
                      bits::span{reinterpret_cast<const quint8 *>(&size), sizeof(size)}, bits::hostOrder());
...
std::optional<quint16> obj::getBootFlagsAddress(const ELFIO::elfio &elf) {
  ...
  for (int it = 0; it < noteAc.get_notes_num(); it++) {
    // Check that note exists and is from me.
    if (!noteAc.get_note(it, noteType, name, desc, descSize)) continue;
    else if (name != "pepp.boot") continue;
    auto descSpan = bits::span<const quint8>{reinterpret_cast<const quint8 *>(desc), descSize};
    auto addr = bits::memcpy_endian<ELFIO::Elf64_Addr>(
        descSpan.first(std::min<std::size_t>(descSize, bootFlagAddrSize)), bits::Order::BigEndian);
    auto size = bits::memcpy_endian<ELFIO::Elf64_Addr>(
        descSpan.subspan(bootFlagAddrSize, std::min<quint64>(descSize - bootFlagAddrSize, bootFlagSizeSize)),
        bits::Order::BigEndian);
    quint64 tmp = 0;
    bits::memcpy_endian(
        bits::span<quint8>{reinterpret_cast<quint8 *>(&tmp), static_cast<size_type>(size)}, bits::hostOrder(),
        bits::span<const quint8>{reinterpret_cast<const quint8 *>(&addr), sizeof(addr)}, bits::hostOrder());
    ret = tmp;
    break;
  }
  return ret;
