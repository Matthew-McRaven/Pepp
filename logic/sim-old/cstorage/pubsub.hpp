...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>) class Channel
    : public std::enable_shared_from_this<Channel<offset_t, val_size_t>> {
    /**
     ...
     * You must poll memory yourself. This design decision, while suboptimal, stems from the lack of an event loop
     * inside Pep/10. In Pep/9, we used Qt's signal/slot system to deliver push-based updates. Inside of the WebASM
     * runtime, we do not have this luxury, mandating pull updates.
     *
  ...
  protected:
    ...
    // Advance one logical timestep and return that event. One logical timestep differs from following a single
    // event->next_node. If the node is marked as empty, then next node is fetched until a non-empty node is found. This
    ...
    // Step back one logical timestep and return that event. One logical timestep differs from following a single
    // event->prev_node. If the node is marked as empty, then next node is fetched until a non-empty node is found. This
    // "skip" mechanism allows for deeply nested reverts.
  ...
  protected:
    struct Event {
        // You should not instantiate this class directly. Rather, you should create one through
        // Channel::append_value(). append_value() will ensure that class invariants are mantained. Events are always
        // appended to the end of the state graph, so next_node is always init'ed to nullptr. All other fields cannot be
        // automatically derived.
        Event(val_size_t value, publisher_id_t publisher, bool empty, std::shared_ptr<Event> prev_node,
              size_t displacement);
