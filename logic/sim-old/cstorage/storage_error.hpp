...
enum class StorageErrc {
    ...
    ResizeError, // Attempted to resize a component which may not be resized. This is recoverable by creating additional
                 // instances of the class.
    Unwritable,  // Write failed because the device doesn't support writing. The value written was ignored. This is
                 // recoverable.
    ...
    IllegalInsert,    // Inserting a storage into a layer failed because an offset is not contained within
                      // 2<<sizeof(offset_t). This is not recoverable.
    ResizeTooSmall,   // Resizing a layered device would cause a chip to have unreachable offsets. This is not
                      // recoverable.
    NoSuchDevice, // When searching for a sub-device, the pointer passed was not a member of the layered storage. This
                  // is not recoverable.
};
...
template <> struct quick_status_code_from_enum<StorageErrc> : quick_status_code_from_enum_defaults<StorageErrc> {
    ...
    static const std::initializer_list<mapping> &value_mappings() {
        static const std::initializer_list<mapping> v = {
            ...
            {StorageErrc::IllegalInsert,
             "Insert into layered device would cause unreachable offsets",
             {errc::invalid_argument}},
            {StorageErrc::ResizeTooSmall,
             "Resizing layered device would create unreachable offsets",
             {errc::invalid_argument}},
