...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    components::storage::Layered<offset_t, enable_history, val_size_t>::Layered(offset_t max_offset,
                                                                                val_size_t default_value,
                                                                                ReadMiss read_policy,
                                                                                WriteMiss write_policy)
requires(enable_history)
    : components::storage::Base<offset_t, enable_history, val_size_t>(max_offset), _storage({}),
      _default_value(default_value), _read_policy(read_policy), _write_policy(write_policy) {}
...
template <typename offset_t, bool enable_history, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    result<void> components::storage::Layered<offset_t, enable_history, val_size_t>::append_storage(offset_t offset,
                                                                                                    storage_t storage) {
    if (offset + storage->max_offset() > (2 << (8 * sizeof(offset_t))))
        return status_code(StorageErrc::IllegalInsert);
...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    result<offset_t> components::storage::Layered<offset_t, enable_history, val_size_t>::storage_to_offset(
        const components::storage::Base<offset_t, enable_history, val_size_t> *to_find)
const {
    for (auto &[offset, storage] : _storage)
        if (storage.get() == to_find)
            return offset;
...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<
         val_size_t>) auto components::storage::Layered<offset_t, enable_history, val_size_t>::contained_storage() const
...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>) void components::
    storage::Layered<offset_t, enable_history, val_size_t>::clear(val_size_t fill_val) {
    for (auto &[offset, storage] : _storage)
        storage->clear(fill_val);
...
template <typename offset_t, bool enable_history, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    result<val_size_t> components::storage::Layered<offset_t, enable_history, val_size_t>::get(offset_t offset)
const {
...
template <typename offset_t, bool enable_history, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    result<void> components::storage::Layered<offset_t, enable_history, val_size_t>::set(offset_t offset,
                                                                                         val_size_t value) {
    if (offset > this->_max_offset)
...
template <typename offset_t, bool enable_history, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    result<val_size_t> components::storage::Layered<offset_t, enable_history, val_size_t>::read(offset_t offset)
const {
    if (offset > this->_max_offset)
        return oob_read_helper(offset);
    else {
...
template <typename offset_t, bool enable_history, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    result<void> components::storage::Layered<offset_t, enable_history, val_size_t>::write(offset_t offset,
                                                                                           val_size_t value) {
    if (offset > this->_max_offset)
        return oob_write_helper(offset, value);
    else {
...
template <typename offset_t, bool enable_history, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>) result < const
    components::storage::Base<offset_t, enable_history, val_size_t>
...
template <typename offset_t, bool enable_history, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>) result <
    components::storage::Base<offset_t, enable_history, val_size_t>
...
template <typename offset_t, bool enable_history, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<
         val_size_t>) bool components::storage::Layered<offset_t, enable_history, val_size_t>::deltas_enabled() const {
...
template <typename offset_t, bool enable_history, typename val_size_t>
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    result<std::unique_ptr<components::delta::Base<offset_t, val_size_t>>> components::storage::Layered<
    ...
    if constexpr (enable_history) {
        ...
        for (auto &[_, storage] : _storage)
            ret->add_delta(std::move(storage->take_delta().value()));
        return {std::move(ret)};
    } else {
        return status_code(StorageErrc::DeltaDisabled);
    }
...
requires(components::storage::UnsignedIntegral<offset_t> &&components::storage::Integral<val_size_t>)
    ...
    size_t max_offset = 0;
    for (auto &[offset, storage] : _storage)
        max_offset = std::max(max_offset, std::size_t(offset) + storage->max_offset());
    if (max_offset > new_offset)
        return status_code(StorageErrc::ResizeTooSmall);
