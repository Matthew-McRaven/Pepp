...
quint8 opcode(Mnemonic mnemonic) {
    return static_cast<quint8>(mnemonic);
}

template <typename AddressingMode, typename Mnemonic>
quint8 opcode(Mnemonic mnemonic, AddressingMode addr) {
    using M = Mnemonic;
...
AddressingMode parseAddressingMode(const QString &addr) {
    using AM = AddressingMode;
    bool ok = true;
...
template <typename Mnemonic>
Mnemonic parseMnemonic(const QString &mnemonic) {
    bool ok = true;
