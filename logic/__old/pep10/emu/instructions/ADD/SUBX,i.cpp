...
TEST_CASE("Instruction: SUBX,i", "[isa::pep10]") {
    ...
    // RTL: X ← X - Oprnd ; N ← X < 0 , Z ← X = 0 , V ← {overflow} , C ← {carry}
    SECTION("SUBX, i") {
        ...
        for (auto target_reg_val : target_reg_values) {
            for (uint16_t opspec = 0; static_cast<uint32_t>(opspec) + 1 < 0x1'0000; opspec++) {
                ...
                std::vector<uint8_t> program = {0xB8, static_cast<uint8_t>((opspec >> 8) & 0xff),
                ...
                CHECK(machine->read_register(isa::pep10::Register::IS) == 0xB8);
