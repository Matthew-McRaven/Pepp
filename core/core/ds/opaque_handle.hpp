...
// consteval void allow_opaque_handle_add(MYTYPE);
template <typename Handle>
concept HandleCanAdd = requires(Handle h) { allow_opaque_handle_add(h); };
...
template <class Tag, class Underlying = u32> struct OpaqueHandle {
  ...
  friend constexpr OpaqueHandle &operator++(OpaqueHandle &h) noexcept
  ...
  friend constexpr OpaqueHandle operator++(OpaqueHandle &h, int) noexcept
    requires HandleCanIncrement<OpaqueHandle>
  {
    OpaqueHandle old = h;
    ++h.value;
  ...
  // Add an integer to a handle
  friend constexpr OpaqueHandle operator+(OpaqueHandle rhs, Underlying lhs) noexcept
    requires HandleCanAdd<OpaqueHandle>
  {
