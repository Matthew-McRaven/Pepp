...
struct IdealClock : ClockSource {
  IdealClock(Device::Descriptor desc, u64 period) : ClockSource(std::move(desc)), _sched({.period = period}) {}
...
struct ScaledClock : public ClockSource {
  ...
  ScaledClock(Device::Descriptor desc, std::shared_ptr<ClockSource> parent, float period_scale,
      ...
      : ClockSource(std::move(desc)), _parent(std::move(parent)), _period_scale(period_scale) {
...
struct MuxClock : public ClockSource {
  // Connect to clock index 0 by default.
  template <typename... Choices>
  explicit MuxClock(Device::Descriptor desc, Choices &&...choices)
      : ClockSource(std::move(desc)), _index(0), _choices{std::forward<Choices>(choices)...} {
