...
SCINTILLA_EXPORT const char *CharacterSetID(Scintilla::CharacterSet characterSet);

SCINTILLA_EXPORT inline QColor QColorFromColourRGBA(ColourRGBA ca) {
...
SCINTILLA_EXPORT inline PRectangle PRectFromQRect(QRect qr) {
  return PRectangle(qr.x(), qr.y(), qr.x() + qr.width(), qr.y() + qr.height());
}

#ifdef PLAT_QT_QML
SCINTILLA_EXPORT inline PRectangle PRectFromQRectF(QRectF qr) {
  return PRectangle(qr.x(), qr.y(), qr.x() + qr.width(), qr.y() + qr.height());
}
#endif

SCINTILLA_EXPORT inline Point PointFromQPoint(QPoint qp) { return Point(qp.x(), qp.y()); }

SCINTILLA_EXPORT inline QPointF QPointFFromPoint(Point qp) { return QPointF(qp.x, qp.y); }

SCINTILLA_EXPORT constexpr PRectangle RectangleInset(PRectangle rc, XYPOSITION delta) noexcept {
  return PRectangle(rc.left + delta, rc.top + delta, rc.right - delta, rc.bottom - delta);
}

class SCINTILLA_EXPORT SurfaceImpl : public Surface {
private:
...
public:
  ...
  void Init(bool, PainterID pid) override;
  ...
  void DrawTextNoClip(PRectangle rc, const Font *font, XYPOSITION ybase, std::string_view text, ColourRGBA fore,
                      ColourRGBA back) override;
  void DrawTextClipped(PRectangle rc, const Font *font, XYPOSITION ybase, std::string_view text, ColourRGBA fore,
                       ColourRGBA back) override;
  void DrawTextTransparent(PRectangle rc, const Font *font, XYPOSITION ybase, std::string_view text,
  ...
  void DrawTextClippedUTF8(PRectangle rc, const Font *font_, XYPOSITION ybase, std::string_view text, ColourRGBA fore,
                           ColourRGBA back) override;
  void DrawTextTransparentUTF8(PRectangle rc, const Font *font_, XYPOSITION ybase, std::string_view text,
                               ColourRGBA fore) override;
