 ...
 */
class SCINTILLA_EXPORT PointDocument {
public:
  double x;
  double y;
...
class SCINTILLA_EXPORT BidiData {
public:
...
class SCINTILLA_EXPORT LineLayout {
private:
...
public:
  ...
  void SetBracesHighlight(Range rangeLine, const Sci::Position braces[], char bracesMatchStyle, int xHighlight,
                          bool ignoreStyle);
...
struct SCINTILLA_EXPORT ScreenLine : public IScreenLine {
  const LineLayout *ll;
  size_t start;
  size_t len;
  XYPOSITION width;
...
struct SCINTILLA_EXPORT SignificantLines {
  Sci::Line lineCaret;
  Sci::Line lineTop;
...
class SCINTILLA_EXPORT LineLayoutCache {
public:
private:
  Scintilla::LineCache level;
...
class SCINTILLA_EXPORT Representation {
public:
  static constexpr size_t maxLength = 200;
  std::string stringRep;
  RepresentationAppearance appearance;
  ColourRGBA colour;
  explicit Representation(std::string_view value = "",
                          RepresentationAppearance appearance_ = RepresentationAppearance::Blob)
      : stringRep(value), appearance(appearance_) {}
};

typedef std::map<unsigned int, Representation> MapRepresentation;

SCINTILLA_EXPORT const char *ControlCharacterString(unsigned char ch) noexcept;
SCINTILLA_EXPORT void Hexits(char *hexits, int ch) noexcept;

class SCINTILLA_EXPORT SpecialRepresentations {
...
public:
  ...
  bool ContainsCrLf() const noexcept { return crlf; }
  bool MayContain(unsigned char ch) const noexcept { return startByteHasReprs[ch] != 0; }
  void Clear();
  void SetDefaultRepresentations(int dbcsCodePage);
};

struct SCINTILLA_EXPORT TextSegment {
  int start;
  int length;
  ...
  int end() const noexcept { return start + length; }
};

// Class to break a line of text into shorter runs at sensible places.
class SCINTILLA_EXPORT BreakFinder {
  const LineLayout *ll;
...
public:
  ...
  BreakFinder(const LineLayout *ll_, const Selection *psel, Range lineRange_, Sci::Position posLineStart,
              XYPOSITION xStart, BreakFor breakFor, const Document *pdoc_, const SpecialRepresentations *preprs_,
...
class SCINTILLA_EXPORT IPositionCache {
public:
  virtual ~IPositionCache() = default;
  virtual void Clear() noexcept = 0;
  virtual void SetSize(size_t size_) = 0;
  virtual size_t GetSize() const noexcept = 0;
  virtual void MeasureWidths(Surface *surface, const ViewStyle &vstyle, unsigned int styleNumber, bool unicode,
                             std::string_view sv, XYPOSITION *positions, bool needsLocking) = 0;
};

SCINTILLA_EXPORT std::unique_ptr<IPositionCache> CreatePositionCache();
