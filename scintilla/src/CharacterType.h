...
constexpr bool IsSpaceOrTab(int ch) noexcept { return (ch == ' ') || (ch == '\t'); }

constexpr bool IsControl(int ch) noexcept { return ((ch >= 0) && (ch <= 0x1F)) || (ch == 0x7F); }

constexpr bool IsEOLCharacter(int ch) noexcept { return ch == '\r' || ch == '\n'; }
...
SCINTILLA_EXPORT int CompareCaseInsensitive(const char *a, const char *b) noexcept;
SCINTILLA_EXPORT int CompareNCaseInsensitive(const char *a, const char *b, size_t len) noexcept;
} // namespace Scintilla::Internal
