...
<h3>
By Sean Eron Anderson<br>
seander&#64cs.<!-- NO SPAM! -->stanford.edu
</h3>

<small>Individually, the <strong>code snippets here are in the 
public domain</strong> 
(unless otherwise noted) &mdash; feel free to use them however you please.  
The aggregate collection and descriptions are 
&copy;&nbsp;1997-2005 Sean Eron Anderson.  
The code and descriptions are distributed 
in the hope that they will be useful,
but <strong>WITHOUT ANY WARRANTY</strong> and 
...
verification system</a>.  What he hasn't tested, I have checked against all
possible inputs on a 32-bit machine.
<strong>To the first person to inform me of a legitimate bug 
in the code, I'll pay a bounty of US$10 (by check or Paypal)</strong>.
If directed to a charity, I'll pay US$20.  </small>
...
<ul>
  ...
  <li><a href="#DetectOppositeSigns">Detect if two integers have opposite signs</li>
  ...
  <li>Counting bits set
    ...
    <li><a href="#CountBitsSetKernighan">Counting bits set, Brian Kernighan's way</a></li>
...
Unfortunately, this method has been <a href=
...
Vincent Lef&egrave;vre convinced me
...
evaluates (x &lt; y).  On July 9, 2008 Vincent Lef&egrave;vre pointed out
...
speed tests on an AMD Athlon&trade; XP 2100+ indicated it was 5-10% 
...
the patterns for the <em>Binary Magic Numbers,</em> B and S.  
...
by <a href="http://onezero.org">Andrew Shapira</a>; 
he found it in pages 187-188 of
<a href="http://www.amd.com/us-en/assets/content_type/white_papers_and_tech_docs/25112.PDF">Software 
Optimization Guide for AMD Athlon&trade; 64 and Opteron&trade; Processors</a>.
...
Juha J&auml;rvi sent this to me on November 21, 2009 as an inverse operation 
...
<p>
Juha J&auml;rvi sent this to me on November 21, 2009.
<p>
...
This swaps the values of a and b <em>without using a temporary variable.</em>
The initial check for a and b being the same location in memory may be
...
Vincent Lef&egrave;vre pointed out the potential for overflow exceptions on
July 9, 2008
...
suppose we have have b = <b>001</b>0<b>111</b>1 
...
mantissa while the exponent is set to 2<sup>52</sup>.  
...
on April 4, 2006.  Vincent Lef&egrave;vre told me on July 9, 2008 to 
...
The third variation was suggested to me by 
<a href="http://www.ece.ucdavis.edu/~jowens/">John Owens</a> 
on April 24, 2002; it's faster, but 
<em>it is only suitable when the input is known to be a power of 2</em>.
...
below to <a href="#RoundUpPowerOf2">round up to a power of 2</a> and 
the method below for 
<a href="#ZerosOnRightMultLookup">computing the number of trailing bits
with a multiply and lookup</a> using a DeBruijn sequence.
...
log<sub>10</sub>(v) = log<sub>2</sub>(v) / log<sub>2</sub>(10), we need to
multiply it by 1/log<sub>2</sub>(10), which is approximately 1233/4096, or
...
<em>subnormal</em> (also called <em>denormal</em>) 
...
subnormal numbers.  On June 11, 2005, Falk H&uuml;ffner pointed out that 
...
On June 11, 2005, Falk H&uuml;ffner pointed out that 
...
it is roughly three times slower than the 
<a href="#RoundUpPowerOf2">technique below</a> 
(which involves 12 operations) when 
benchmarked on an Athlon&trade; XP 2100+ CPU.  Some CPUs will fare
...
may be best.  (On a Athlon&trade; XP 2100+ I've found the above shift-left 
...
<p>
There is yet a faster method &mdash; 
use <a href="#HasLessInWord"><code>hasless</code></a>(v, 1), 
...
Juha J&auml;rvi later suggested <code>hasless(v, 1)</code>
...
<p>
Juha J&auml;rvi sent this clever technique to me on April 6, 2005.  The
...
<p>
The macro <code>hasmore</code> was suggested by Juha J&auml;rvi on 
...
<p>
Juha J&auml;rvi suggested <code>likelyhasbetween</code> on April 6, 2005.  
From there,
...
A Belorussian translation</a> (provided by <a
href="http://webhostingrating.com/">Webhostingrating</a>)
is available.
