...
TEST_CASE("Microassemble 1-byte bus", "[scope:mc2][kind:unit][arch:*]") {
  ...
  SECTION("Microcoded control section") {
    ...
    auto result = pepp::ucode::parse<uarch2c, regs>(source);
    CHECK(result.errors.empty());
    REQUIRE(result.program.size() == 1);
  ...
  SECTION("Yay formatting") {
    {
      ...
      REQUIRE(result.program.size() == 1);
      auto &line = result.program[0];
      auto text = pepp::ucode::format<uarch, regs>(line);
    ...
    }
    {
      ...
      REQUIRE(result.program.size() == 1);
      auto &line = result.program[0];
      auto text = pepp::ucode::format<uarch2c, regs>(line);
  ...
  SECTION("Valid pre/post") {
    {
      ...
      CHECK(line.type == pepp::ucode::ParseResult<uarch, regs>::Line::Type::Post);
    ...
    }
    {
      ...
      auto &line = result.program[0];
      CHECK(line.controls.enables.count() == 0);
      CHECK(line.type == pepp::ucode::ParseResult<uarch, regs>::Line::Type::Pre);
      CHECK(line.tests.size() == 2);
    }
  }
  ...
  SECTION("Test register value out-of-range") {
    {
      ...
      auto result = pepp::ucode::parse<uarch, regs>(good_source);
      CHECK(result.errors.size() == 0);
      CHECK(result.program.size() == 1);
      QString bad_source = "UnitPost: T1=0x100";
      result = pepp::ucode::parse<uarch, regs>(bad_source);
      CHECK(result.errors.size() == 1);
      CHECK(result.program.size() == 0);
    }
    {
