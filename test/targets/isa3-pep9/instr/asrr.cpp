...
template <isa::Pep9::Register target_reg> void inner(isa::Pep9::Mnemonic op) {
  ...
  DYNAMIC_SECTION("with initial value " << init_reg) {
    auto endRegVal = static_cast<quint16>(init_reg >> 1) | (init_reg & 0x8000 ? 1 << 15 : 0);
    ...
    CHECK(csr(cpu, isa::Pep9::CSR::C) == (init_reg & 0x1));
  }
}
} // namespace

TEST_CASE("Pep9::Mnemonic::ASRA", "[scope:targets][kind:int][target:pep9]") {
  using Register = isa::Pep9::Register;
  inner<Register::A>(isa::Pep9::Mnemonic::ASRA);
}
TEST_CASE("Pep9::Mnemonic::ASRX", "[scope:targets][kind:int][target:pep9]") {
  using Register = isa::Pep9::Register;
  inner<Register::X>(isa::Pep9::Mnemonic::ASRX);
}
