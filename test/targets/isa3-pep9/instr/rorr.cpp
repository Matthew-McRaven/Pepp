...
template <isa::Pep9::Register target_reg> void inner(isa::Pep9::Mnemonic op) {
  ...
  DYNAMIC_SECTION("with initial value==" << init_reg << " and carry==" << carry) {
    ...
    cpu->csrs()->write(static_cast<quint8>(::isa::Pep9::CSR::C), {&carry, 1}, rw);
    ...
    CHECK(csr(cpu, isa::Pep9::CSR::Z) == 0);
    auto new_reg = reg(cpu, target_reg);
    CHECK(csr(cpu, isa::Pep9::CSR::V) == 0);
    // Carry out if low order bit was non-zero
    CHECK(csr(cpu, isa::Pep9::CSR::C) == quint16(init_reg & 0x1));
  }
}
} // namespace

TEST_CASE("Pep9::Mnemonic::RORA", "[scope:targets][kind:int][target:pep9]") {
  using Register = isa::Pep9::Register;
  inner<Register::A>(isa::Pep9::Mnemonic::RORA);
}
TEST_CASE("Pep9::Mnemonic::RORX", "[scope:targets][kind:int][target:pep9]") {
  using Register = isa::Pep9::Register;
  inner<Register::X>(isa::Pep9::Mnemonic::RORX);
}
