...
namespace {
int randInt(int min, int max) { return min + (rand() % static_cast<int>(max - min + 1)); }
unsigned randUpto(unsigned max) { return rand() % max; }
...
static std::tuple<Allocation, size_t> realloc_random(riscv::Arena &arena, uint64_t addr) {
  ...
  const Allocation a{.addr = newaddr, .size = arena.size(newaddr)};
  REQUIRE(a.size >= size);
  return {a, size};
}

TEST_CASE("RISC-V basic heap usage", "[scope:sim][kind:int][arch:RV]") {
...
TEST_CASE("RISC-V heap allocate too many chunks", "[scope:sim][kind:int][arch:RV][!throws]") {
  REQUIRE_THROWS([] {
    riscv::Arena arena{BEGIN, END};
    while (true) arena.malloc(4);
  }());
