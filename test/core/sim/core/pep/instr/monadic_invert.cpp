...
void inner_not(PepISA3CPU::ISA isa, Register target_reg, Register other_reg, Mnemonic op) {
  ...
  DYNAMIC_SECTION("with initial value " << init_reg) {
    auto endRegVal = static_cast<u16>(~init_reg);
...
template <typename Register, typename CSR, typename Mnemonic>
void inner_neg(PepISA3CPU::ISA isa, Register target_reg, Register other_reg, Mnemonic op) {
  auto [sys, mem, cpu] = make_cpu(isa);
  // Loop over a subset of possible values for the target register.
  u16 tmp;
  auto [init_reg] = GENERATE(table<u16>({0, 1, 0x7fff, 0x8000, 0x8FFF, 0xFFFF}));
  DYNAMIC_SECTION("with initial value " << init_reg) {
    ...
    CHECK(csr(cpu, CSR::N) == (endRegVal & 0x8000 ? 1 : 0));
    CHECK(!!csr(cpu, CSR::Z) == (endRegVal == 0));
    CHECK(!!csr(cpu, CSR::V) == (endRegVal == 0x8000));
...
TEST_CASE("(new) Pep/9, NEGX", "[scope:core][scope:core.sim][kind:unit][arch:pep10]") {
  ...
  using CSR = isa::Pep9::CSR;
  using MN = isa::Pep9::Mnemonic;
  inner_neg<Register, CSR, MN>(PepISA3CPU::ISA::Pep9, Register::X, Register::A, MN::NEGX);
}
