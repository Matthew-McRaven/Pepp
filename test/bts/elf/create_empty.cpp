...
TEST_CASE("Test custom ELF library, 32-bit", "[scope:elf][kind:unit][arch:*]") {
  ...
  SECTION("Emit a working .gnu.version_r") {
    ...
    {
      PackedVersionNeedWriter<ElfBits::b32, ElfEndian::le> vs_writer(elf, versymr_idx);
...

    {
      ...
      PackedVersionDefWriter<ElfBits::b32, ElfEndian::le> vd_writer(elf, versymd_idx);
      auto add_def = [&](std::uint16_t ndx, std::uint16_t flags, std::uint16_t cnt, std::uint32_t name_off,
                         std::string_view name_sv) {
        PackedElfVerdef<ElfEndian::le> vd{};
        vd.vd_version = 1;
        vd.vd_flags = flags;
        vd.vd_ndx = ndx;
        vd.vd_cnt = cnt;
      ...
      auto add_aux = [&](std::size_t def_index, std::uint32_t name_off) {
        PackedElfVerdaux<ElfEndian::le> a{};
        a.vda_name = name_off;
